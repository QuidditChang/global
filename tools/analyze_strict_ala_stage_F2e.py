#!/usr/bin/env python3
"""Fail-closed Stage-F2e L4 two-element face-patch analysis."""
...
CANDIDATES = ("CONFIGURED", "L4_FACE_Z", "L4_FACE_X", "L4_FACE_Y")
BLOCKS = {
...
def rows_identical(current_rows, prior_rows, keys, numbers,
                   current_name, prior_name):
    if len(current_rows) != len(prior_rows):
        return False
    current_rows = sorted(current_rows, key=lambda row: tuple(row[k] for k in keys))
    prior_rows = sorted(prior_rows, key=lambda row: tuple(row[k] for k in keys))
    for current, prior in zip(current_rows, prior_rows):
        if any(current[key] != prior[key] for key in keys):
            return False
        if any(not math.isclose(
                finite(current[key], f"{current_name} {key}"),
                finite(prior[key], f"{prior_name} {key}"),
                rel_tol=1e-10, abs_tol=1e-14) for key in numbers):
            return False
    return True


def analyze(args):
    lineage = load_json(args.f2d_decision)
    ...
    prior_trajectory = read_csv(args.f2d_trajectory, trajectory_columns)
    prior_levels = read_csv(args.f2d_levels, (
        "candidate", "POD_mode", "POD_energy_weight", "MG_cycle",
        "top_level", "level", "phase", "input_rms", "output_rms",
        "reduction_ratio", "alpha", "valid"))
    prior_rhs = read_csv(args.f2d_rhs,
                         ("POD_mode", "POD_energy_weight", "rhs_norm", "valid"))
    ...
    trajectory_keys = {
        (row["candidate"], int(row["POD_mode"]), int(row["MG_cycles"]))
        for row in trajectory}
    trajectory_valid = (
        len(trajectory) == len(expected) and trajectory_keys == expected and
        all(row["valid"] == "1" and row["candidate"] in CANDIDATES and
            ...
            0.0 < finite(row["finest_damping"], "damping") <= 1.0 and
            finite(row["momentum_residual_relative"], "residual") >= 0.0
            for row in trajectory))
    ...
    prior_baseline = [row for row in prior_trajectory
                      if row["candidate"] == "CONFIGURED"]
    baseline_identity = rows_identical(
        current_baseline, prior_baseline,
        ("candidate", "POD_mode", "MG_cycles", "valid"),
        ("finest_sweeps", "finest_damping", "POD_energy_weight",
         "momentum_residual_relative"), "F2e", "F2d")

    current_levels = [row for row in levels if row["candidate"] == "CONFIGURED"]
    old_levels = [row for row in prior_levels if row["candidate"] == "CONFIGURED"]
    baseline_level_identity = rows_identical(
        current_levels, old_levels,
        ("candidate", "POD_mode", "MG_cycle", "top_level", "level",
         "phase", "valid"),
        ("POD_energy_weight", "input_rms", "output_rms",
         "reduction_ratio", "alpha"), "F2e", "F2d")

    rhs_identity = rows_identical(
        mode_rhs, prior_rhs, ("POD_mode", "valid"),
        ("POD_energy_weight", "rhs_norm"), "F2e", "F2d")
    ...
    level_valid = all(
        ...
        finite(row["input_rms"], "level input") >= 0.0 and
        finite(row["output_rms"], "level output") >= 0.0 and
        finite(row["reduction_ratio"], "level ratio") >= 0.0
        for row in levels)
    max_top = max(int(row["top_level"]) for row in levels)
    final_l4 = [row for row in levels if
                row["phase"] == "DOWN_SMOOTH" and
    ...
    performance_valid = (
        ...
        all(row["valid"] == "1" and
            ...
            finite(row["seconds_max"], "seconds") > 0.0
            for row in performance))

    cache_by = {row["orientation"]: row for row in cache}
    cache_valid = (
        len(cache) == 3 and set(cache_by) == {"face_z", "face_x", "face_y"} and
        all(row["valid"] == "1" and int(row["patches_per_cap"]) > 0 and
            int(row["block_dof"]) == 36 and
            0.0 < finite(row["cache_mb_max"], "cache MB") <= 64.0 and
            finite(row["build_seconds_max"], "build seconds") > 0.0 and
            finite(row["min_pivot_ratio"], "pivot ratio") > 0.0
            for row in cache))

    lineage_ok = (
        lineage.get("experiment_evidence_valid") is True and
        lineage.get("decision") == "F2D_NO_ROBUST_L4_SMOOTHER_CANDIDATE" and
        lineage.get("next_authorized_task") ==
            "F2E_FACE_PATCH_BLOCK_REDESIGN" and
        lineage.get("production_default_change_authorized") is False)
    configured_sweeps = runtime.get("configured_finest_sweeps")
    configured_damping = finite(
    ...
    frozen_controls = (
        ...
        all(math.isclose(finite(row["finest_damping"], "damping"),
                         configured_damping, rel_tol=0.0, abs_tol=1e-15)
            for row in trajectory))

    gates = {
        "F2d_lineage": lineage_ok,
        "runtime_schema":
            runtime.get("schema") == "strict-ala-stage-F2e-runtime-v1",
        "frozen_current_viscosity":
            runtime.get("viscosity") == "frozen_current",
        "frozen_operator":
        ...
        "face_patch_structure": runtime.get("candidate_structure") ==
            "nonoverlapping_two_element_face_pairs",
    ...
    for candidate in CANDIDATES:
        final = [row for row in trajectory if
                 row["candidate"] == candidate and
                 int(row["MG_cycles"]) == 64]
        at32 = [row for row in trajectory if
                row["candidate"] == candidate and
        ...
        if candidate != "CONFIGURED":
            ...
            item.update({
                ...
                "cache_mb_max": finite(cache_row["cache_mb_max"], "cache MB"),
                "build_seconds_max": finite(
                    cache_row["build_seconds_max"], "build seconds"),
                "min_pivot_ratio": finite(
    ...
    if not structural_pass:
        decision = "F2E_INVALID_STRUCTURAL_EVIDENCE"
        next_task = "REPEAT_F2E_VALID_EXPERIMENT"
        selected_candidate = None
    elif qualified:
        selected_candidate = min(
            qualified, key=lambda name: metrics[name]["momentum_residual_RMS_64"])
        decision = "F2E_FACE_PATCH_CANDIDATE_IDENTIFIED"
        next_task = "F2F_PRODUCTION_VELOCITY_AB"
    else:
        selected_candidate = None
        decision = "F2E_NO_ROBUST_FACE_PATCH_CANDIDATE"
        next_task = "F2F_OVERLAPPING_OR_CROSS_RANK_BLOCK_REDESIGN"
    write_json(args.output, {
        "schema": "strict-ala-stage-F2e-decision-v1",
...
def audit(args):
    ...
    write_json(args.output, {
        "schema": "strict-ala-stage-F2e-final-audit-v1",
        ...
        "next_authorized_task": decision.get("next_authorized_task") if valid
            else "REPEAT_F2E_VALID_EXPERIMENT",
    })
    return 0
...
def parser():
    ...
    for name in (
            "f2d-decision", "f2d-trajectory", "f2d-levels", "f2d-rhs",
