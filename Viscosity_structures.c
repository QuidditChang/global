...
static void low_viscosity_channel_factor1(struct All_variables *E, float *F);
     ...
     float **evisc,**visc;
{
    ...
    void visc_from_PB();
    void record_tracer_visc();
    void read_tracer_visc();
          ...
          for(kkk=1;kkk<=E->lmesh.noy;kkk++)
            for(jjj=1;jjj<=E->lmesh.nox;jjj++){
                ...
                for(iii=E->lmesh.noz;iii>=1;iii--)  {
                    ...
                    else if(r<0.91 && r>0.88) {
                        tempa = E->viscosity.N0[2] + (E->viscosity.N0[3]-E->viscosity.N0[2])*(cos((r-0.88)/0.03*3.14159)+1)/2;
                        viscE = E->viscosity.E[2] + (E->viscosity.E[3]-E->viscosity.E[2])*(cos((r-0.88)/0.03*3.14159)+1)/2;
                        viscT = E->viscosity.T[2] + (E->viscosity.T[3]-E->viscosity.T[2])*(cos((r-0.88)/0.03*3.14159)+1)/2;
                    }
                    else {
                        tempa = E->viscosity.N0[l-1];
			   ...
			   if(E->control.tracer && r>0.99 && E->trace.ntracer_flavor[m][1][i]>0.0 && E->age_t[nodeg]>180.0/E->data.scalet) {
			      if(fabs(E->asm_depth[nodeg])*1000.0/6371.0<0.03)
				temp = one;
                           }

                           EEta[m][ (i-1)*vpts + jj ] = tempa*exp(viscE/(temp+viscT) - viscE/(one+viscT) );

                           temp1 = 200*tempa; flag=1.0;
			   fi = E->long_t[m][node];
                           if(EEta[m][(i-1)*vpts+jj]>temp1 && flag && r>0.92 && r<1.1)
                              if(fabs(E->asm_depth[nodeg])*1000.0/6371.0<0.1){
                                  if(E->asm_depth[nodeg]<0.0 || (E->asm_depth[nodeg]>0.0 && r<0.99))
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)  {
      if(E->viscosity.sdepv_visited){
        ...
        //if(E->parallel.me == 0) {
           ...
           /*fprintf(E->fp,"mesh.elx=%d,mesh.ely=%d,mesh.elz=%d\n",E->mesh.elx,E->mesh.ely,E->mesh.elz);
           fprintf(E->fp,"lmesh.nox=%d,lmesh.noy=%d,lmesh.noz=%d\n",E->lmesh.nox,E->lmesh.noy,E->lmesh.noz);
           fprintf(E->fp,"lmesh.elx=%d,lmesh.ely=%d,lmesh.elz=%d\n",E->lmesh.elx,E->lmesh.ely,E->lmesh.elz);
           fprintf(E->fp,"lmesh.exs=%d,lmesh.eys=%d,lmesh.ezs=%d\n",E->lmesh.exs,E->lmesh.eys,E->lmesh.ezs);*/

           fflush(E->fp);
       // }
        if(t2 > 1e2) {
           for(e=1;e<=nel;e++)
             eedot[e] = eedot[e]/t2*100;
        }
     ...
     float **EEta, **oldEEta;
{
    ...
    float theta,phi,fi,rad,age,v_trench;
    float find_age_in_MY();
    double t1,t2;
	...
	rad = 0.5*(E->sx[m][3][E->ien[m][e].node[1]]+E->sx[m][3][E->ien[m][e].node[8]]);
        ...
        fi = E->long_t[m][E->ien[m][e].node[1]];
	  ...
	  if(fabs(fi)<50.0 && E->asm_depth[nodeg]<0.0 && fabs(E->asm_depth[nodeg])*1000.0/6371.0<0.15)
	     tau = 0.2 * E->viscosity.pdepv_y[l];

	  /* increased yld stress if within the cold slab */
	  if(E->T[m][E->ien[m][e].node[1]]<E->control.lith_age_mantle_temp)
	     tau = 10.0 * E->viscosity.pdepv_y[l];
	  ...
	  eta_p = tau/(2.0 * eedot[e] + 1e-7); /* + E->viscosity.pdepv_offset; */
	   ...
	   /* yielding within the cold slab */
            if(E->T[m][E->ien[m][e].node[1]]<E->control.lith_age_mantle_temp && rad>0.97) {
		t1 = fi-0.015;
		if(fabs(fi)<50.0 && E->asm_depth[nodeg]*1000.0/6371.0<0.045 && E->asm_depth[nodeg]*1000.0/6371.0>-0.005) {
		    if(E->asm_depth[nodeg]*1000.0/6371.0<0.045) t2 = 0.03;
		    if(E->asm_depth[nodeg]*1000.0/6371.0>-0.005) t2 = 0.02;
		    //EEta[m][(e-1)*vpts + jj] = eta_new+(EEta[m][(e-1)*vpts+jj]-eta_new)*(1.0-cos(pow(fabs(phi-t1)/t2,3)*1.57));
		}
            }

	   /* Yielding within compostional field (background p=0) */
            for(p=1; p<=E->composition.ncomp; p++) {  /* PDEPV not apply to background mantle (composition 0) */
                 if(E->trace.ntracer_flavor[m][p][e] > 0.0 && rad>0.97) {
		   if((fi<50.0 && E->asm_depth[nodeg]*1000.0/6371.0<0.05 && E->asm_depth[nodeg]*1000.0/6371.0>-0.05) && rad<0.999)
 ...
 *          element */
	    ...
	    fi = E->long_t[m][E->ien[m][i].node[1]];
	   /* if(fi<50.0) {
              if(rad>0.99 && phi>fi-0.05 && phi<=fi+0.05 && E->trace.ntracer_flavor[m][1][i]>0.0)
		  EEta[m][ (i-1)*vpts + jj ] *= vmean;
	      else if(rad>0.97 && phi>fi+0.05)
		  EEta[m][ (i-1)*vpts + jj ] *= vmean;
	      else if(rad<0.997)
		  EEta[m][ (i-1)*vpts + jj ] *= vmean;
	    }
	    ...
	    EEta[m][ (i-1)*vpts + jj ] *= vmean;
               ...
               /*if(E->sx[m][2][E->ien[m][i].node[1]]>fi-0.02 && E->trace.ntracer_flavor[m][1][i]>0.0)
                   EEta[m][ (i-1)*vpts + jj ] *= vmean;
               else if(E->sx[m][2][E->ien[m][i].node[1]]>fi-0.02)
		   EEta[m][ (i-1)*vpts + jj ] *= vmean;
                   //EEta[m][ (i-1)*vpts + jj ] = v_now + (EEta[m][(i-1)*vpts+jj]-v_now)*(cos((E->sx[m][3][E->ien[m][i].node[1]]-0.96)/0.04*3.1416)+1)/2.0;
               if(E->trace.ntracer_flavor[m][2][i]>0.0) {
		  if(E->sx[m][3][E->ien[m][i].node[1]]>0.97)
                     EEta[m][ (i-1)*vpts + jj ] = v_now;
	       }
	       else
		   EEta[m][ (i-1)*vpts + jj ] = v_now;*/

	       /* define low-viscosity channel above slab*/
	       /*if(E->trace.ntracer_flavor[m][1][i]>0.0 && E->sx[m][3][E->ien[m][i].node[1]]>0.97)
		 if(E->mesh.topvbc == 3.0)
		   EEta[m][ (i-1)*vpts + jj ] = 0.01;
		 */  

	    /* read in tracer-recorded low-viscosity structure in continent */
            for(p=5;p<E->composition.ncomp;p++) {
			...
			EEta[m][(i-1)*vpts + jj] = oldEEta[m][(i-1)*vpts+jj];
                   /* partial swap */
                   /*EEta[m][(i-1)*vpts + jj] = 1.0/(1.0/oldEEta[m][ (i-1)*vpts+jj] + 1.0/v_now);*/
                   /*if(E->sx[m][2][E->ien[m][i].node[1]]>fi-0.02 && E->trace.ntracer_flavor[m][1][i]>0.0)
                      EEta[m][(i-1)*vpts + jj] = min(v_now,EEta[m][(i-1)*vpts + jj]);*/
               } 
            }  /* end of p loop */

            /*if(E->monitor.solution_cycles > 0 && E->trace.ntracer_flavor[m][0][i]==0.0) {
               EEta[m][ (i-1)*vpts + jj ] = oldEEta[m][(i-1)*vpts+jj];
               if(E->sx[m][2][E->ien[m][i].node[1]]>fi-0.02 && E->trace.ntracer_flavor[m][1][i]>0.0)
                  EEta[m][(i-1)*vpts + jj] = min(v_now,EEta[m][(i-1)*vpts + jj]);
            }*/
...
static void apply_low_visc_wedge_channel(struct All_variables *E, float **evisc)
{
    ...
    if(E->viscosity.channel) {
        low_viscosity_channel_factor1(E, F);
...
static void low_viscosity_channel_factor1(struct All_variables *E, float *F)
{
    int i, ii, k, m, e, ee, count1,count2;
    int nz_min[NCS], nz_max[NCS];
    const int flavor = 1;
    double rad_mean, rr, width=E->viscosity.lv_channel_thickness, flag;
    float phi_mean,fi,visc1,visc2;
    ...
    for(m=1; m<=E->sphere.caps_per_proc; m++) 
		...
		if(flag == 1.0 && count1>=3) {
		    flag = 2.0;
		    count2 = 0;
		}

		/* add a low viscosity channel above slab */
		if(flag == 2.0 || (fabs(fi)<50.0 && phi_mean<fi+0.01 && phi_mean>fi-0.05 && flag==1.0)) {  
		  //flag = 3.0;
                  /* loop over elements above e */
                  for(ii=i; ii<=E->lmesh.elz-1; ii++) {
			   ...
			   count2 += 1;
			   if(count2==1 && flag==2.0)
                              F[ee] = 10*E->viscosity.lv_reduction;
			   if(count2==2 && flag==2.0)
                              F[ee] = 3*E->viscosity.lv_reduction;
			   if(count2>=3 && flag==2.0)
                              F[ee] = E->viscosity.lv_reduction;
                         }
                    } /* end of ii loop */
		    flag = 3.0;
		} /* end of if statement */

		/* add a weak mantle wedge above r=0.97 */
		if(flag == 3.0 && fabs(fi)<50.0 && phi_mean>fi && phi_mean<fi+0.1) {
		  //flag = 0.0;
                  for(ii=i; ii<=E->lmesh.elz-1; ii++) {
		    ee = (k-1)*E->lmesh.elz + ii;
		    rr = 0.5 * (E->sx[m][3][E->ien[m][ee].node[1]] +
			...
			F[ee] = visc1+(1.0-visc1)*cos(pow((fi+0.1-phi_mean)/0.1,3)*1.57);
		    } 
		  }
		}   /* end of weak mantle wedge */
...
void find_trench_location(struct All_variables *E)
{
  ...
  if(E->sphere.caps == 1) {
     sprintf(pb_1,"/work/02505/qzhou1/Real_Earth/WUS/NAM/Nam_subduct/subduction_sL.%0.0f.xy",age1);
     sprintf(pb_2,"/work/02505/qzhou1/Real_Earth/WUS/NAM/Nam_subduct/subduction_sL.%0.0f.xy",age2);
  }
...
{
    int    m,e,i,j,k,jj,node,nodeg,surf,wedge,flag;
    int    bd_i,bd_j,bd_k,nox,noy,noz;
    float  phi,fi,rad,age;
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)
	  ...
	  flag = 0;
	    ...
	    for(jj=1;jj<=vpts;jj++) {
	      ...
	      if(surf && fabs(fi)<50.0 && fabs(E->asm_depth[nodeg])*1000.0/6371.0<width && rad>(1.0-height)) {
	  	 //EEta[m][(e-1)*vpts+jj] = 0.01;
		// tt1 = phi-((fi+0.02)+7.0*(1.0-rad));
		 tt1=-E->asm_depth[nodeg]*1000.0/6371.0-0.02-7.0*(1-rad);
		 if(tt1>-0.08 && tt1<0.0) {
		   tt2 = 0.01+(1.0-(1*cos((pow(tt1/0.08,3)*1.57))));
		   tt3 = cos(pow((1.0-rad)/height,3)*1.5708);
		   for(jj=1;jj<=vpts;jj++) {
	 	      visc = EEta[m][(e-1)*vpts+jj];
		      tt1 = visc + (tt2-visc)*tt3;
		      if(visc>tt1) 
			EEta[m][(e-1)*vpts+jj] = tt1;
		   }
		 }
	      } /* end of weak surface */

	      /* add a weak mantle wedge above slab */
	//      if(wedge && fabs(fi)<50.0 && phi>fi-0.03 && phi<fi+wide_wdg && rad>d_wdg) {
	      if(wedge  && E->asm_depth[nodeg]*1000.0/6371.0<0.03 && E->asm_depth[nodeg]*1000.0/6371.0>-wide_wdg && rad>d_wdg) {
 //                if(phi-((fi-0.03)+(1.0-rad))>0.0) {
		 if(-E->asm_depth[nodeg]*1000.0/6371.0+0.03-(1.0-rad)>0.0) {
			 ...
			 temp1 = (1.0-E->T[m][node]/E->control.lith_age_mantle_temp)*1.0;
                         temp1 += (EEta[m][(e-1)*vpts+jj]-temp1)*cos(pow((rad-d_wdg)/(1.0-d_wdg),0.5)*1.57);
                         if(-E->asm_depth[nodeg]*1000.0/6371.0>wide_wdg*3/4.0)
                             temp1 += (EEta[m][(e-1)*vpts+jj]-temp1)*cos(pow((wide_wdg-(-E->asm_depth[nodeg]*1000.0/6371.0))/(wide_wdg/4.0),1)*1.57);
                         if(temp1 < 0.01)
                             temp1 = 0.01;
