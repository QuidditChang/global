...
{
    int i,j,m,p;
    double temp,temp1,temp2,depth;
    void remove_horiz_ave2(struct All_variables*, double**);

    /* Rayleigh number */
    temp = E->control.Atemp;

    //fprintf(stderr,"refstate.rho=%f, refstate.thermal_expansivity=%f",E->refstate.rho[10],E->refstate.thermal_expansivity[10]);
    //fflush(stderr);
    ...
    for(i=1;i<=E->lmesh.nno;i++) {
         ...
         */
         //if(E->trace.ntracer_flavor[m][0][i] > 0.0) //no buoyancy if outside air layer
            buoy[m][i] =  temp * E->refstate.rho[nz]
                * E->refstate.thermal_expansivity[nz] * E->T[m][i];
         //else
           // buoy[m][i] =  temp * E->refstate.rho[nz]
             //   * E->refstate.thermal_expansivity[nz] * E->control.lith_age_mantle_temp;

	/* remove thermal buoyancy if in the air-layer */
	/*for(p=1;p<E->composition.ncomp;p++) {
            if(E->trace.ntracer_flavor[m][p][i]>0.0)
                buoy[m][i] *= 0.0;
        }*/
	...
	/* Lijun add */
	temp1 = fabs(1.0-E->sx[m][3][i]);
        depth = 0.00;
        //if(E->age_t[nodeg]>180.0/E->data.scalet) depth *= 2;
        if(temp1<depth && E->data.timedir>0) { // && E->composition.comp_node[m][1][i]>0.0) {// && E->age_t[nodeg]<180.0/E->data.scalet) {
            temp2 = 1e10*cos((temp1-0.00)/(depth-0.00)*1.57);
	    buoy[m][i] += temp2 * E->refstate.rho[nz]
		* E->refstate.thermal_expansivity[nz] 
		* (E->T[m][i]-E->control.lith_age_mantle_temp);
		//* (E->control.lith_age_mantle_temp-E->T[m][i]);
	}
    ...
    if(E->control.tracer &&
       (E->composition.ichemical_buoyancy)) {
        //fprintf(E->fp,"icompositional_rheology=%d\n",E->composition.ichemical_buoyancy);
        //fprintf(E->fp,"composition.ncomp=%d\n",E->composition.ncomp);
        //fprintf(E->fp,"buoyancy_ratio[0]=%f,buoyancy_ratio[1]=%f\n",E->composition.buoyancy_ratio[0],E->composition.buoyancy_ratio[1]);
        //fflush(E->fp);
