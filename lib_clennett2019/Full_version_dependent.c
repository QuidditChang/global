...
void full_construct_tic_from_input(struct All_variables *E)
{
  ...
  if (E->parallel.me==0)
            fprintf(stderr,"Tic_method choice!\n");
  ...
  case 1: //Read in the initial temperature field
	sprintf(input_file,"./Restart/slab.velo.%d.0",E->parallel.me);
	...
	if (fp == NULL) {
  	...
  	if (E->parallel.me==0) {
	    fprintf(stderr,"Reading %s for stored velocities\n",input_file);
      	    fprintf(E->fp,"Reading %s for stored velocities\n",input_file);
  	}
  	...
  	for(m=1;m<=E->sphere.caps_per_proc;m++) {
        	...
        	sscanf(input_s,"%g %g %g %f",&(v1),&(v2),&(v3),&(g));
            	...
            	//E->sphere.cap[m].V[2][i] = v2;
            	//E->sphere.cap[m].V[3][i] = v3;
            	E->T[m][i] = g;
	    }
	}
    break;
