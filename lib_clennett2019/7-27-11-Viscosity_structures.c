     ...
     int propogate;
{
    ...
    float phi, theta, rad, fi, r, fi_trench, the_trench;
	...
	for(m=1;m<=E->sphere.caps_per_proc;m++)
		...
		flag2 = 0;
		    ...
		    /* creat a smooth radial visc. profile */
				...
				/*temp1 =100*tempa; // *(1.1-cos((r-0.9)/0.1*1.57));
                                if(EEta[m][(i-1)*vpts+jj]>temp1 && r>0.92 && r<0.99)
			...
			if(rheo_trick == 1) { //a strong slab in the upper mantle as a stress guide
					...
					if (E->sx[m][2][node]>fi && E->sx[m][2][node]<fi+lon_inc1+0.03 && r>0.99){
					    ...
					    temp2 = 7.0; 
					    fi = 0.5-2.0*(3.14159/(11.1*180.0))*(200.0-age);
					    //if(age<191.0) 
					    ...
					    //	fprintf(E->fp,"fi=%f\n",fi);
                                            ...
                                            dist1 = 0.08;
                                            if(temp1>-dist1 && temp1<0.03 && r>0.985) { // && theta<=lat_max+0.015 && theta>=lat_min+0.005) {
						...
						temp2 = 0.01+(1.0-(1*cos((pow(temp1/dist1,0.3)*1.57))));
                                                if(EEta[m][(i-1)*vpts+jj]>temp2) { // && theta<=lat_max-0.005 && theta>=lat_min+0.0) {
                                                  if(r>0.99)
                                                     EEta[m][(i-1)*vpts+jj]=temp2*5*cos((r-0.99)/0.01*1.57);
                                                  else {
                                                     temp1 = EEta[m][(i-1)*vpts+jj];
                                                     temp2 = temp2*5+ (temp1-temp2*5)*(0.99-r)/(0.99-0.98);
                                                     EEta[m][(i-1)*vpts+jj] = temp2;
                                                  }
                                                }
                                            } //end of weak top
					    ...
					    temp2 = 7.0;
                                            ...
                                            dist1 = 0.03;
                                            if(temp1>0.0-dist1 && temp1<0.03 && r>0.98 && phi>fi-0.03){
                                                if(temp1<0.0) temp2 = (cos(pow(temp1/dist1,3)*3.1416)+1)/2;
                                                if(temp1>=0.0) temp2 = (cos(pow(temp1/0.03,3)*3.1416)+1)/2;
