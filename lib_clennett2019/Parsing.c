...
//  if((strstr(str,"on")!=NULL) || (strstr(str,"ON")!=NULL))
  if((strstr(str,"on")) || (strstr(str,"ON")))
    *value=1;
//  else if ((strstr(str,"off") != NULL) || (strstr(str,"OFF")!=NULL))
  else if ((strstr(str,"off")) || (strstr(str,"OFF")))
    *value=0;
...
//  if (strstr(interpret,"essential")!=NULL)
  if (strstr(interpret,"essential"))
   { *essential=1; /* no default possible, must read a value */
     return(0);
   }

//  if (strstr(interpret,"nodefault")==NULL)
  if (!strstr(interpret,"nodefault"))
   { if((strstr(interpret,"on")) || (strstr(interpret,"ON")))
       *Default = 1.0;
     else
       if ((strstr(interpret,"off")) || (strstr(interpret,"OFF")))
	 *Default = 0.0;
       else
