     ...
     float **evisc,**visc;
{
    ...
    */
    ...
    /* Lijun add: interpolate from element to node, for advection of viscosity */
    visc_from_gint_to_nodes(E,E->oldEVI[E->mesh.levmax],E->oldVI[E->mesh.levmax],E->mesh.levmax);
          ...
          for(kkk=1;kkk<=E->lmesh.noy;kkk++)
            for(jjj=1;jjj<=E->lmesh.nox;jjj++){
                ...
                for(iii=E->lmesh.noz;iii>=1;iii--)  {
                    ...
                    else if(r<0.96 && r>0.93) {
                        tempa = E->viscosity.N0[1] + (E->viscosity.N0[2]-E->viscosity.N0[1])*(cos((r-0.93)/0.03*3.14159)+1)/2;
                        viscE = E->viscosity.E[1] + (E->viscosity.E[2]-E->viscosity.E[1])*(cos((r-0.93)/0.03*3.14159)+1)/2;
                        viscT = E->viscosity.T[1] + (E->viscosity.T[2]-E->viscosity.T[1])*(cos((r-0.93)/0.03*3.14159)+1)/2;
                    }
     ...
     float **EEta,**oldEEta;
{
    ...
    /* Lijun add: record viscosity for the next time step */
    for(m=1;m<=E->sphere.caps_per_proc;m++)  {
       for(e=1;e <= nel;e++)  { /* loop through all elements */
          for(jj=1;jj <= vpts;jj++){ /* loop through integration points */
	...
	for(jj=1;jj <= vpts;jj++){ /* loop through integration points */
	  ...
	  //eedot[e],tau,eta_p,eta_new,EEta[m][(e-1)*vpts + jj]);
 	  ...
 	  /* Lijun change to single flavor function */
            if (E->control.tracer && E->trace.ntracer_flavor[m][0][e] > 0.0) // number of flavor 0 tracers per elem.
	       EEta[m][(e-1)*vpts + jj] = eta_new;
	    ...
	    if(E->sx[m][3][E->ien[m][i].node[1]]>0.96) {
		   ...
		   EEta[m][ (i-1)*vpts + jj ] *= vmean;
            ...
            for(p=1; p<E->composition.ncomp; p++)
                if(E->trace.ntracer_flavor[m][p][i] > 0.0 && E->monitor.solution_cycles > 0) {
                   EEta[m][(i-1)*vpts + jj] = pow((EEta[m][ (i-1)*vpts + jj ] * oldEEta[m][(i-1)*vpts+jj]),0.5);
                   /*EEta[m][(i-1)*vpts + jj] = min(EEta[m][ (i-1)*vpts + jj ],oldEEta[m][(i-1)*vpts+jj]);*/
            } /* end of p loop */
