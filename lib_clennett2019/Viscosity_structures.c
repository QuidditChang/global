...
//void find_trench_location(struct All_variables*);

static void apply_low_visc_wedge_channel(struct All_variables *E, float **evisc);
static void low_viscosity_channel_factor1(struct All_variables *E, float *F, float **evisc);
static void low_viscosity_channel_factor2(struct All_variables *E, float *F);
static void low_viscosity_wedge_factor(struct All_variables *E, float *F);
static void record_tracer_visc();
     ...
     float **evisc,**visc;
{
    ...
    void weak_plate_boundary_from_velocities_v2();
    void weak_slab_hinge_wedge();
    void apply_smooth_viscbc();
    ...
    if(E->viscosity.CDEPV && E->monitor.solution_cycles > 5)	/* compositional prefactor */
      visc_from_C(E,evisc,E->oldEVI[E->mesh.levmax]);

    if(E->viscosity.SDEPV)
      visc_from_S(E,evisc,propogate);

    /*if(E->control.tracer && E->monitor.solution_cycles > 0) 
      read_tracer_visc(E,evisc);*/

    if(E->viscosity.PDEPV && E->monitor.solution_cycles > 20)	/* "plasticity" */
      visc_from_P(E,evisc,E->oldEVI[E->mesh.levmax]);
...
/*    if(E->control.tracer)
    ...
    if((E->viscosity.channel || E->viscosity.wedge) && E->monitor.solution_cycles > 10)
        apply_low_visc_wedge_channel(E, evisc);

    /*if(E->mesh.topvbc == 1) {
       apply_smooth_viscbc(E,evisc);
    }*/
    ...
    //if(E->viscosity.CDEPV && E->monitor.solution_cycles > 5)    /* compositional prefactor */
      //visc_from_C2(E,evisc,E->oldEVI[E->mesh.levmax]);
    ...
    /* interpolate from gauss quadrature points to node points for output, i.e., viscosity filtering */
    for(i=1;i<=2;i++) {
    ...
    /* update internal VBC */
    if(E->monitor.solution_cycles>0 && E->mesh.topvbc>=3) {
       regional_velocity_boundary_conditions(E);
       construct_id(E);
    }
...
void visc_from_T(E,EEta,propogate)
     ...
     int propogate;
{
    int m,i,j,k,l,z,p,jj,kk,imark,iii,jjj,kkk,intage,node,nodeg,nnn,ttt,count,bd_i,bd_j,bd_k;
          ...
          for(kkk=1;kkk<=E->lmesh.noy;kkk++)
            for(jjj=1;jjj<=E->lmesh.nox;jjj++){
                ...
                for(iii=E->lmesh.noz;iii>=1;iii--)  {
                    ...
                    else if(r<0.874431 && r>=0.6075969) {
                        tempa = 10 + (100-10)*(cos((r-0.6075969)/(0.874431-0.6075969)*3.14159)+1)/2;
                        viscE = E->viscosity.E[l-1];
                        viscT = E->viscosity.T[l-1];
                    }
                    else if(r<0.89640558 && r>=0.874431) {
                        tempa = 1 + (10-1)*45.5071428*(0.89640558-r);
                        viscE = E->viscosity.E[l-1];
                        viscT = E->viscosity.T[l-1];
                    }
		        ...
		        viscT = E->viscosity.T[l-1];
                    ...
                    else if(r<0.89640558 && r>=0.6075969) {
                        tempa = 10 + (100-10)*(cos((r-0.6075969)/(0.89640558-0.6075969)*3.14159)+1)/2;
                        viscE = E->viscosity.E[l-1];
                        viscT = E->viscosity.T[l-1];
                    }*/
		    ...
		    /* Jiashun add, remove overshoot and undershoot */
		    /*if(E->T[m][node]>=1.1*E->control.lith_age_mantle_temp) {
                         E->T[m][node] = 1.1*E->control.lith_age_mantle_temp;
                    }
		    ...
		    if(E->T[m][node]<=-0.01) {
			   ...
			   if(temp < one){  /* enhance the weak viscosity of plume */
			       //if(r<0.98)
                                  EEta[m][ (i-1)*vpts + jj ] = tempa*exp(3.5/(temp+viscT) - 3.5/(one+viscT) );
			       ...
			       //else
                           ...
                           if(EEta[m][(i-1)*vpts+jj]>temp1 && flag && r>0.92 && r<1.1 && fabs(E->flag_depth2[nodeg])*1000.0/6371.0<0.1)
                              if(fabs(E->flag_depth2[nodeg])*1000.0/6371.0<0.1){
                                  if(E->flag_depth2[nodeg]>0.0)
				      temp1 = 25.0+75.0*cos((fabs(E->flag_depth2[nodeg])*1000.0/6371.0-0.1)/0.1*3.1415926/2.0);
                        ...
                        /*fprintf(E->fp,"cycle=%d, iii=%d, rad=%f, e=%d, node=%d, EEta[%d]=%f\n",E->monitor.solution_cycles,iii,r,i,node,jj,EEta[m][(i-1)*vpts+jj]);
                        fflush(E->fp); */
...
void visc_from_PB(E,EEta,propogate)
	struct All_variables *E;
     float **EEta;
     int propogate;
{
	...
	for(m=1;m<=E->sphere.caps_per_proc;m++)
		    ...
		    /*for(lv=E->mesh.gridmax;lv>=E->mesh.gridmin;lv--) {
		      if(r>0.982 && r<0.985) { */
                     ...
                     /*    if(lv==E->mesh.gridmax) {
                     	    E->sphere.cap[m].VB[1][node] = E->velo_1[nodeg];
                     	    E->sphere.cap[m].VB[2][node] = E->velo_2[nodeg];
		     	 } // lv statement 
                       } // r statement
                    }  */  // for loop
			...
			if(l<3) 
                        ...
                        if(theta > E->control.theta_max-0.2) 
                            if(theta > E->control.theta_max-0.15) {
                                temp2 = temp1 + (temp-temp1)*(cos(pow((theta-(E->control.theta_max-0.15))/0.15,10)*3.14159)+1)/2;
                                EEta[m][(i-1)*vpts + jj] = temp2;
                            }
                            else {
                                temp2 = temp1 + (temp-temp1)*(cos(pow((theta-(E->control.theta_max-0.15))/0.05,3)*3.14159)+1)/2;
                                if(EEta[m][(i-1)*vpts + jj] > temp2) 
                                  EEta[m][(i-1)*vpts + jj] = temp2;
                            }
                        if(theta < E->control.theta_min+0.2) 
                            if(theta < E->control.theta_min+0.15) {
                                temp2 = temp1+(temp-temp1)*(cos(pow((E->control.theta_min+0.15-theta)/0.15,10)*3.14159)+1)/2;
                                EEta[m][(i-1)*vpts + jj] = temp2;
                            }
                            else {
                                temp2 = temp1+(temp-temp1)*(cos(pow((E->control.theta_min+0.15-theta)/0.05,3)*3.14159)+1)/2;
                                if(EEta[m][(i-1)*vpts + jj] > temp2)
                                  EEta[m][(i-1)*vpts + jj] = temp2;
                            } 
                        if(phi > E->control.fi_max-0.2)
                            if(phi > E->control.fi_max-0.15) {
                                temp2 = temp1+(temp-temp1)*(cos(pow((phi-(E->control.fi_max-0.15))/0.15,10)*3.14159)+1)/2;
                                ...
                                temp2 = temp1+(temp-temp1)*(cos(pow((phi-(E->control.fi_max-0.15))/0.05,3)*3.14159)+1)/2;
                                if(EEta[m][(i-1)*vpts + jj] > temp2)
                                  EEta[m][(i-1)*vpts + jj] = temp2;
                            }
                        if(phi < E->control.fi_min+0.2)
                           if(phi < E->control.fi_min+0.15) {
                                temp2 = temp1+(temp-temp1)*(cos(pow((E->control.fi_min+0.15-phi)/0.15,10)*3.14159)+1)/2;
                                if(theta <= E->control.theta_max-0.03 && theta >= E->control.theta_min+0.03)
                                  EEta[m][(i-1)*vpts + jj] = temp2;
                           }
                           else if(theta <= E->control.theta_max-0.05 && theta >= E->control.theta_min+0.05){
                                temp2 = temp1+(temp-temp1)*(cos(pow((E->control.fi_min+0.15-phi)/0.05,3)*3.14159)+1)/2;
                                if(EEta[m][(i-1)*vpts + jj] > temp2)
                                  EEta[m][(i-1)*vpts + jj] = temp2;
                            } */


			/* add a plume at the left end of the box */
			/*if(fabs(r-0.95)<0.04 && fabs(phi-0.25)<0.04 && fabs(age-E->control.start_age)<0.5){
                            dist1 = sqrt(pow((r-0.95),2)+pow((phi-0.25),2));
                            temp2 = 0.15*(cos(dist1/0.04*3.1416)+1)/2.0;
			    if(dist1 <= 0.04)
                               E->T[m][node] = E->control.lith_age_mantle_temp+temp2;
			}*/

			/* add a weak top layer above oceanic plate */
			/*if(E->age_t[nodeg]>0.0 && E->sx[m][3][E->ien[m][i].node[1]]>0.997)
			  if(E->mesh.topvbc == 3.0) 
      ...
      /*if(E->mesh.topvbc == 1) { // && E->monitor.solution_cycles>0) {
     ...
     if(E->viscosity.sdepv_visited){
      ...
      for(e=1;e<=nel;e++){
       ...
       for(e=1;e<=nel;e++)   {
           ...
           for(jj=1;jj<=vpts;jj++) {
               t1 = scale*pow(EEta[m][(e-1)*vpts+jj],exponent1)*adj_factor;
               if(E->viscosity.sdepv_expt[E->mat[m][e]-1]!=1)
		 //if(E->trace.ntracer_flavor[m][0][e] > 0.0)
                   EEta[m][(e-1)*vpts + jj] = t1*EEta[m][(e-1)*vpts + jj]/(t1+EEta[m][(e-1)*vpts + jj]);
...
void visc_from_S0(E,EEta,propogate)
     struct All_variables *E;
     float **EEta;
     int propogate;
{
	...
	// (Lijun put) Formula: eta = pow(eta,1/n)*pow(epcilon_dot,1/n-1)
        for(e=1;e<=nel;e++)   {
            ...
            scale=pow(eedot[e],exponent1-one);
            for(jj=1;jj<=vpts;jj++) {
		t1 = scale*pow(EEta[m][(e-1)*vpts+jj],exponent1);
		EEta[m][(e-1)*vpts + jj] = t1*EEta[m][(e-1)*vpts + jj]/(t1+EEta[m][(e-1)*vpts + jj]);
	    }
        }
    }
...
void visc_from_S1(E,EEta,propogate)
     struct All_variables *E;
     float **EEta;
     int propogate;
{
...
void visc_from_P(E,EEta,oldEEta)
     ...
     float **EEta, **oldEEta;
{
    ...
    /*if(E->monitor.solution_cycles == 0) {
        oldEEta=(float*) malloc((nel*vpts+1)*sizeof(float));
    }*/
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)  {
      ...
      /* end of Lijun addition */


      for(e=1;e <= nel;e++)   { /* loop through all elements */

        l = E->mat[m][e] -1 ;   /* material of this element */
	node = E->ien[m][e].node[1];
        iii = node/(E->lmesh.nox*E->lmesh.noz)+1;
        jjj = (node-(iii-1)*E->lmesh.nox*E->lmesh.noz)/E->lmesh.noz+1;
        nodeg = E->lmesh.nxs-1+jjj+(E->lmesh.nys+iii-2)*E->mesh.nox;
	rad = 0.5*(E->sx[m][3][E->ien[m][e].node[1]]+E->sx[m][3][E->ien[m][e].node[8]]);
        phi = 0.5*(E->sx[m][2][E->ien[m][e].node[1]]+E->sx[m][2][E->ien[m][e].node[8]]);
	//dist = E->flag_depth2[nodeg]*1000.0/6371.0;
	...
	age1 = E->age_t[nodeg]*E->data.scalet;
	  ...
	  eta_p = tau/(2.0 * eedot[e] + 1e-7); /* + E->viscosity.pdepv_offset; */
	   ...
	   /* yielding within the cold slab */
	    ...
	    if(fabs(dist)<0.05) 
	      oldEEta[m][ (e-1)*vpts + jj ] = 1.0;

        /*fprintf(E->fp,"e=%d, EEta[%d]=%f\n",e,jj,EEta[m][(e-1)*vpts+jj]);*/
...
void visc_from_P2(E,EEta) /* "plasticity" implementation
     ...
     float **EEta;
{
	...
	for(jj=1;jj <= vpts;jj++){ /* loop through integration points */
	  ...
	  //eedot[e],tau,eta_p,eta_new,EEta[m][(e-1)*vpts + jj]);
 	  ...
 	  /* Lijun change to single flavor function */
            if (E->control.tracer && E->trace.ntracer_flavor[m][1][e] > 0.0) // number of flavor 0 tracers per elem.
	       EEta[m][(e-1)*vpts + jj] = eta_new;
	       //EEta[m][(e-1)*vpts + jj] = tau;
	  /* end of Lijun's change */
...
*/
void visc_from_C1( E, EEta)
     struct All_variables *E;
     float **EEta;
{
  float comp,comp_fac,CC[9],tcomp,comp_mean;
  double vmean,cc_loc;
  int m,l,z,jj,kk,i,p,flavor_mean;
  ...
  for(m=1;m <= E->sphere.caps_per_proc;m++)  {
	...
	if(CC[kk] > 1) CC[kk]=1.0;
      ...
      for(p=0;p<E->trace.nflavors;p++) {
         if(E->trace.ntracer_flavor[m][p][i]>comp_mean) {
              flavor_mean = p;
          }
      } /* end p loop */
      fprintf(E->fp,"i=%d,flavor=%d\n",i,flavor_mean);
      fflush(E->fp);  
	...
	/* geometric mean of viscosity */
        vmean = exp(cc_loc  * E->viscosity.cdepv_ff[flavor_mean] +
...
void visc_from_C2(E,EEta,oldEEta)
     struct All_variables *E;
     float **EEta,**oldEEta;
{
  double vmean,v_now,cc_loc[10],CC[10][9],cbackground;
  int m,l,z,jj,kk,i,ii,p,q,node,nodeg;
  float v_trench,phi,rad,age,dist,T_m,tmp_eff;
  float find_age_in_MY();
  FILE *fp;
  ...
  T_m = E->control.lith_age_mantle_temp;

  fp=fopen("velo_trench","r");
    if(fp==NULL)
        v_trench = 2.0;
    else {
        fscanf(fp,"%f",&(v_trench));
        fclose(fp);
  }
  ...
  for(m=1;m <= E->sphere.caps_per_proc;m++)  {
    for(i = 1; i <= nel; i++){
        ...
        age=E->age_t[nodeg]*E->data.scalet;
	dist = fabs(E->flag_depth2[nodeg])*1000.0/6371.0;
      ...
      /* determine composition of each of the nodes of the element */
        ...
        for(jj = 1; jj <= vpts; jj++) {
            ...
            for(p=0; p<E->composition.ncomp; p++) {
		if(p>=8)
		    tmp_eff = E->viscosity.cdepv_ff[p+1];
		else
		    tmp_eff = 0.0;
                vmean += cc_loc[p] * tmp_eff;
            }
            vmean = exp(vmean);
            ...
            /* multiply the viscosity with this prefactor */
	    rad = 0.5*(E->sx[m][3][E->ien[m][i].node[1]]+E->sx[m][3][E->ien[m][i].node[8]]);
	    phi = 0.5*(E->sx[m][2][E->ien[m][i].node[1]]+E->sx[m][2][E->ien[m][i].node[8]]);
		    
	    /* read in tracer-recorded low-viscosity structure in continent */
            for(p=8;p<E->composition.ncomp;p++) {
               if(E->trace.ntracer_flavor[m][p][i]>0.0 && E->monitor.solution_cycles>0 && E->sx[m][3][E->ien[m][i].node[1]]>0.89) {
                   EEta[m][ (i-1)*vpts + jj ] *= vmean;
               } 
            }  /* end of p loop */
...
void visc_from_C(E,EEta,oldEEta)
     struct All_variables *E;
     float **EEta,**oldEEta;
{
  double vmean,v_now,cc_loc[30],CC[30][9],cbackground;
  int m,l,z,jj,kk,i,ii,p,q,node,nodeg;
  float v_trench,phi,rad,age,T_m;
  float dist,flag_depth;
  float find_age_in_MY();
  const int oc=1,obc=2,ol=3,opl=4,oecl=5,weakarc=6,cuc=7,clc=8,cul=9,cml=10,cll=11,cratuc=12,cratlc=13,cratul=14,cratml=15,cratll=16,cecl=17,chem=18;
  FILE *fp;
	   ...
	   // else if(rad>0.896)
	    ...
	    /* read in tracer-recorded low-viscosity structure in continent */
            /*for(p=10;p<E->composition.ncomp;p++) {
               if(E->trace.ntracer_flavor[m][p+1][i]>0.0 && E->monitor.solution_cycles>0 && E->sx[m][3][E->ien[m][i].node[1]]>0.96) {
                   if(EEta[m][(i-1)*vpts + jj] > oldEEta[m][(i-1)*vpts+jj])
			EEta[m][(i-1)*vpts + jj] = oldEEta[m][(i-1)*vpts+jj];
               } 
            }*/  /* end of p loop */
...
static void apply_low_visc_wedge_channel(struct All_variables *E, float **evisc)
{
    ...
    if(E->viscosity.channel) {
        low_viscosity_channel_factor1(E, F, evisc);
	//low_viscosity_channel_factor2(E, F);
    }
...
static void low_viscosity_channel_factor1(struct All_variables *E, float *F, float **evisc)
{
    int i, ii, k, m, e, ee, count1,count2,iii1,jjj1,nodeg1,node1,iii2,jjj2,nodeg2,node2;
    int nz_min[NCS], nz_max[NCS];
    const int flavor = 1;
    double rad_mean, rr, width=E->viscosity.lv_channel_thickness, flag,flag_depth;
    float phi_mean,visc1,visc2,visc3,dist,age;

    visc1 = E->viscosity.lv_reduction;
    ...
    for(m=1; m<=E->sphere.caps_per_proc; m++)
	    ...
	    flag = 0.0; count1 = count2 = 0;
            for(i=nz_min[m]; i<=nz_max[m]; i++) {
                e = (k-1)*E->lmesh.elz + i;
                node1 = E->ien[m][e].node[1];
            	node2 = E->ien[m][e].node[8];
            	iii1 = node1/(E->lmesh.nox*E->lmesh.noz)+1;
            	jjj1 = (node1-(iii1-1)*E->lmesh.nox*E->lmesh.noz)/E->lmesh.noz+1;
            	nodeg1 = E->lmesh.nxs-1+jjj1+(E->lmesh.nys+iii1-2)*E->mesh.nox;
            	iii2 = node2/(E->lmesh.nox*E->lmesh.noz)+1;
            	jjj2 = (node2-(iii2-1)*E->lmesh.nox*E->lmesh.noz)/E->lmesh.noz+1;
            	nodeg2 = E->lmesh.nxs-1+jjj2+(E->lmesh.nys+iii2-2)*E->mesh.nox;
        	flag_depth=(E->flag_depth2[nodeg1]+E->flag_depth2[nodeg2])/2.0;
                rad_mean = 0.5 * (E->sx[m][3][E->ien[m][e].node[1]] +
                                  E->sx[m][3][E->ien[m][e].node[8]]);

        	phi_mean = 0.5 * (E->sx[m][2][E->ien[m][e].node[1]] +
				  E->sx[m][2][E->ien[m][e].node[8]]);
		dist = (fabs(E->flag_depth2[nodeg1])*1000.0/6371.0+fabs(E->flag_depth2[nodeg2])*1000.0/6371.0)/2.0;
		age = (E->age_t[nodeg1]+E->age_t[nodeg2])*E->data.scalet/2.0;

                /* determine if above the tracer layer (flag = 2.0) */
                if(E->trace.ntracer_flavor[m][flavor][e] > 0.0)
                    if(flag == 0.0) {
                        flag = 1.0;
		 	count1 = 1;  /* # of weak elem above each tracer */
                        count2 += 1; /* # of tracers in the same verticl colume */
                    }
                ...
                if(flag == 1.0 && (dist<0.02 || age>380.0)) {
                  ...
                  for(ii=i; ii<=nz_max[m]; ii++) {
                    ...
                    if(rr>0.965)
                        ...
                        visc3 = evisc[m][(ee-1)*vpoints[E->mesh.nsd] + 1];
                        visc2 = visc1 + (visc3-visc1)*(1.0-(cos(pow((0.965-rr)/(0.965-E->viscosity.lv_min_radius),3)*3.14159)+1)/2.0);
                    }

                    /* if ee is within the channel */
                    if(rr <= rad_mean + E->viscosity.lv_channel_thickness) {
                        if(nz_max[m]-i>=3 && count2 <= 2) {
...
void weak_plate_boundary_from_velocities(E,EEta)
    struct All_variables *E;
    float **EEta; 
{                 
           ...
           if(temp1 > 0.0) {
             ...
             for(i=noz1;i>=1;i--)  {
               ...
               if(r > 0.99) {
                    ...
                    for(jj=1;jj<=vpts;jj++) {
			//fprintf(stderr,"Before: rad=%f,temp2=%f, evisc=%f\n",r,temp2,EEta[m][(el-1)*vpts+jj]);
...
void weak_plate_boundary_from_velocities_v2(E,EEta)
    struct All_variables *E;
    float **EEta;
{
    int m,i,j,k,l,p,q,t,jj,node,nodeg,el,bd_i,bd_j,bd_k,n11,n12,n21,n22;
    int nox,noy,nox1,noy1,noz1,lev,flag;
    float v_grd11,v_grd12,v_grd21,v_grd22,age;
    float theta,phi,r,tempa,temp,temp1,temp2,tf_dist;
    float find_age_in_MY();
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)
      for(k=1;k<=noy1;k++)
	   ...
	   if(fabs(E->flag_depth2[nodeg])<0.1)
		continue;

	   tf_dist = fabs(E->tf_depth[nodeg]);
	   if(tf_dist <= 0.99) {
	     for(i=noz1;i>=1;i--)  {
               if(k == 1)
                 bd_k = 1;
               ...
               if(r >= 0.98) {
                    for(jj=1;jj<=vpts;jj++) {
                    	temp2 = 0.1 + (EEta[m][(el-1)*vpts+jj]-0.1)*(1.0-cos(pow(tf_dist/0.99,5)*3.1415926/2.0)); // + (E->viscosity.N0[1]-0.1)*(1.0-cos(pow((1.0-r)/0.02,3)*1.57));
                        if(EEta[m][(el-1)*vpts+jj]>temp2) {
                            EEta[m][(el-1)*vpts+jj] = temp2;
			}
		    }
               }
...
void viscosity_filter(E,EEta)
    struct All_variables *E;
    float **EEta;
{
    int m,i,j,k,l,p,q,t,jj,node,nodeg,el,bd_i,bd_j,bd_k,n11,n12,n21,n22,n13,n31;
    int elx,ely,elx1,ely1,elz1,lev,flag;
    float v_grd11,v_grd12,v_grd21,v_grd22,age;
    float theta,phi,r,tempa,temp,temp1,temp2;
    float find_age_in_MY();
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)
      for(k=1;k<=ely1;k++)
        for(j=1;j<=elx1;j++)    
          for(i=1;i<=elz1;i++) {
              el = i + (j-1)*elz1 + (k-1)*elz1*elx1;
              n11=el-1; n12=el+1;  /* 3D smoothing */
              /* n11=el; n12=el; */	     /* 2D smoothing */
              n21=el-elz1; n22=el+elz1; 
              n13=el-elz1*elx1; n31=el+elz1*elx1;
              ...
              for(jj=1;jj<=vpts;jj++) {
                 temp1 = (EEta[m][(n11-1)*vpts+jj] + EEta[m][(n12-1)*vpts+jj]
		       +  EEta[m][(n21-1)*vpts+jj] + EEta[m][(n22-1)*vpts+jj]
...
static void record_tracer_visc(E,oldEEta)
      struct All_variables *E;
      float **oldEEta;
{
   int m,e,kk,jj;
   float visc_tracer,theta,phi,rad;
   double weight[9],visc_el[9];

   const int vpts = vpoints[E->mesh.nsd];

   if(E->parallel.me==0)
     fprintf(stderr, "starting record_tracer_visc\n");
   ...
   for(m=1;m<=E->sphere.caps_per_proc;m++)  {
      for(kk=1;kk<=E->trace.ntracers[m];kk++) { /* loop through all tracers in serial */
          ...
          if(e > 0) {
             theta=E->trace.basicq[m][0][kk];
             phi=E->trace.basicq[m][1][kk];
             rad=E->trace.basicq[m][2][kk];

             /* get shape functions at (theta, phi, rad) */
             get_shape_functions(E, weight, e, theta, phi, rad);
  
             /* calculate the viscosity at the position of tracer */
             visc_tracer = 0.0;
             for(jj=1;jj<=vpts;jj++) {
                //fprintf(E->fp,"kk=%d, e=%d, EEta[%d]=%f\n",kk,e,jj,EEta[m][(e-1)*vpts+jj]);
                visc_tracer += oldEEta[m][(e-1)*vpts+jj]*weight[jj];
             }
...
static void read_tracer_visc(E,EEta)
      struct All_variables *E;
      float **EEta;
{
   int m,e,kk,jj;
   ...
   int el_count[nel];
   float visc_count[nel];



   /* set counter and temperary viscosity array to zero */
   for(m=1;m<=E->sphere.caps_per_proc;m++)
     for(e=1;e<=nel;e++) {
        el_count[e] = 0;   
        visc_count[e] = 0.0;
   ...
   /* set average elemental viscosity */
   for(m=1;m<=E->sphere.caps_per_proc;m++)  
      for(e=1;e<=nel;e++) {
            visc_mean = visc_count[e] / el_count[e];
            for(jj=1;jj<=vpts;jj++) {
               EEta[m][(e-1)*vpts+jj] *= visc_mean;
            } /* end of jj loop */
      } /* end of e statement */
...
static void read_tracer_visc22(E,oldEEta,oldVI)
      struct All_variables *E;
      float **oldEEta,**oldVI;
{
   int m,e,kk,jj,j;
   ...
   for(m=1;m<=E->sphere.caps_per_proc;m++)
     for(e=1;e<=nel;e++) {
        ...
        visc_count[e] = 0.0;
        for(j=1;j<=ends;j++)
             oldVI[m][E->IEN[lev][m][e].node[j]] = 0.0;
     }
   ...
   for(m=1;m<=E->sphere.caps_per_proc;m++)
      for(e=1;e<=nel;e++)
         if(el_count[e]>0) {
            visc_mean = visc_count[e] / el_count[e];
            for(j=1;j<=ends;j++) {
               if(oldVI[m][E->IEN[lev][m][e].node[j]] == 0.0)
                  oldVI[m][E->IEN[lev][m][e].node[j]] = visc_mean;
               /*else
                  oldVI[m][E->IEN[lev][m][e].node[j]] = 
                    //pow((oldVI[m][E->IEN[lev][m][e].node[j]]*visc_mean),0.5);
                    (oldVI[m][E->IEN[lev][m][e].node[j]] + visc_mean)/2.0; */
         } /* end of j loop */
      } /* end of if statement */
 
   visc_from_nodes_to_gint(E,oldVI,oldEEta,E->mesh.levmax);
                                ...
                                double theta, double phi, double rad)
{
    ...
    ***/
    ...
    /* compute volumetric weighting functions */
    volume = dx*dz*dy;
...
void find_trench_location(struct All_variables *E)
{
  float temp1,temp2,fi_1,fi_2,fi,*PB1[4],*PB2[4];
  float theta,phi,lat_max,lat_max1,lat_max2,lat_min,lat_min1,lat_min2,age,age1,age2;
  float tt1,tt2,ttt1,ttt2,v_trench;
  ...
  if(E->sphere.caps == 1) {
     sprintf(pb_1,"/home1/01523/lijun/work/jobs/BC_files/Nam_subduct/subduction_sL.%0.0f.xy",age1);
     sprintf(pb_2,"/home1/01523/lijun/work/jobs/BC_files/Nam_subduct/subduction_sL.%0.0f.xy",age2);
  }
      ...
      for(i=1;i<=noy;i++)
        for(m=1;m<=nox;m++)
          for(k=1;k<=noz;k++)  {
            ...
            /* calculate trench velocity */
            v_trench = -100000.0*E->data.scalev;
            temp1 = fi_1-fi_2;
            temp2 = sin(temp1)*sin(theta);
            if(theta<=lat_max+0.05 && theta>=lat_min-0.05 && fabs(phi-fi)<0.15) {
               v_trench = 1.0*637.1*temp2*E->data.scalev;
               /* update back-arc surface velocity */
               if(E->age_t[nodeg]==0.0/E->data.scalet && fabs(v_trench/E->data.scalev)<20) {
                  if(fabs(v_trench)>fabs(E->sphere.cap[j].VB[2][node])) {
                     temp1 = E->sphere.cap[j].VB[2][node];
                     temp2 = temp1 + (v_trench-temp1)*cos((phi-fi)/0.15*1.57);
                     E->sphere.cap[j].VB[2][node] = temp2;
                  }
               }
            }
...
void weak_slab_hinge_wedge(E,EEta)
	struct All_variables *E;
	float **EEta;

{
    int    m,e,i,j,k,ii,jj,node,nodeg,surf,wedge,flag,jjj,fault,mark;
    int    bd_i,bd_j,bd_k,nox,noy,noz;
    float  phi,rad,age,dip,dist1,dist2;
    float  width,height,wide_wdg,d_wdg,temp1,temp2,tt1,tt2,tt3,visc;
    const int oc=1,obc=2,ol=3,opl=4,oecl=5,weakarc=6,cuc=7,clc=8,cul=9,cml=10,cll=11,cratuc=12,cratlc=13,cratul=14,cratml=15,cratll=16,cecl=17,chem=18;
    ...
    if(E->monitor.solution_cycles>200) surf=0;
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)
	  ...
	  flag = 0;
          for(k=noz;k>=1;k--)  {
            ...
            e = (k-bd_k) + (j-bd_j)*(noz-1) + (i-bd_i)*(noz-1)*(nox-1);
            nodeg=E->lmesh.nxs-1 + j +(E->lmesh.nys+i-2)*E->mesh.nox;
            node=k+(j-1)*noz+(i-1)*nox*noz;
            phi = E->sx[m][2][node];
	    rad = E->sx[m][3][node];
	    age = E->age_t[nodeg]*E->data.scalet;
            dist1 = E->flag_depth2[nodeg]*1000.0/6371.0;
	    dist2 = fabs(E->flag_depth2[nodeg])*1000.0/6371.0;

	    for(jj=1;jj<=vpts;jj++) {

	      /* add a weak surface above subduction zone */
	      if(surf && dist2<width && rad>(1.0-height)) {
		 tt1=-dist1-0.0-dip*(1-rad);
		 if(tt1>-width && tt1<width/4.0 && dist1>-0.02) {
                   tt2 = 0.01+(1.0-(1*cos((pow(tt1/width,3)*1.57))));
		   visc = EEta[m][(e-1)*vpts+jj];
		   //if(visc>tt2) 
		   EEta[m][(e-1)*vpts+jj] = tt2;
		 }
	      } /* end of weak surface */

             if(fault && dist1<-0.01 && dist2<0.04 && rad>0.988) {
                tt1 = rad-(0.988+0.02*(sin((dist1+0.01)/0.03*1.57)+1)/2.0);
                if(tt1>-0.0 && tt1<0.02) {
                   tt2 = 0.01+10*(1.0-(cos((pow(tt1/0.02,3)*1.57))));
                   EEta[m][(e-1)*vpts+jj] = tt2;
                }
             }

	      /* add a weak mantle wedge above slab */
	      if(wedge && dist1>-wide_wdg && rad>d_wdg && dist2<0.7) {
		 if(-dist1+0.03-(1.0-rad)>0.0) {
                     if(E->T[m][node]>=0.7*E->control.lith_age_mantle_temp && flag==0) {
			 temp1 = (1.0-E->T[m][node]/E->control.lith_age_mantle_temp)*EEta[m][(e-1)*vpts+jj];
                         temp1 += (EEta[m][(e-1)*vpts+jj]-temp1)*cos(pow((rad-d_wdg)/(1.0-d_wdg),0.2)*1.57);
                         if(-dist1>wide_wdg*3/4.0)
                             temp1 += (EEta[m][(e-1)*vpts+jj]-temp1)*cos(pow((wide_wdg-(-dist1))/(wide_wdg/4.0),1)*1.57);
                         if(temp1 < 0.01)
                             temp1 = 0.01;
			 if (temp1<EEta[m][(e-1)*vpts+jj])
                         EEta[m][(e-1)*vpts+jj] = temp1;
                      }
                      if(E->T[m][node]<0.7*E->control.lith_age_mantle_temp)
                           flag = 1;
                  } /*end of if(phi) statement */
	      } /* end of mantle wedge */
              ...
              /*if(flag<=2*vpts && age>380.0 && dist2>0.02 && E->monitor.solution_cycles>20) {
                 if(E->T[m][node]<=0.95*E->control.lith_age_mantle_temp && E->T[m][node]>=0.6*E->control.lith_age_mantle_temp && rad>0.95) {
                    ...
                    for(ii=cuc;ii<=cratll;ii++)
                        if(E->trace.ntracer_flavor[m][ii][e]>0.0) {
                            mark=1;
                            break;
                        }
                    if(mark==0 && rad<0.985) {
                        EEta[m][(e-1)*vpts+jj] *= 0.001;
                        if(k<noz) EEta[m][(e)*vpts+jj] *= 0.01;
                        if(k<noz-1) EEta[m][(e+1)*vpts+jj] *= 0.1;
                        flag += 1;
                    }
                 }
              }*/ /* end of weak layer*/

              /* add a weak layer atop slab at greater depth below oceans */
              /*if(flag<=2*vpts && age<195.0 && dist1>=0.02 && E->monitor.solution_cycles>20) {
                 if(E->T[m][node]<=0.95*E->control.lith_age_mantle_temp && E->T[m][node]>=0.6*E->control.lith_age_mantle_temp && rad>=0.95) {
                    mark=0;
                    for(ii=cuc;ii<=cratll;ii++)
                        if(E->trace.ntracer_flavor[m][ii][e]>0.0) {
...
void apply_smooth_viscbc(E,EEta)
    struct All_variables *E;
    float **EEta;

{
   int m,i,l,jj,node;
   float temp,temp1,temp2,theta,phi,rad,gap,ramp;
   const int vpts = vpoints[E->mesh.nsd];
   ...
   for(m=1;m<=E->sphere.caps_per_proc;m++)
      for(i=1;i<=E->lmesh.nel;i++) {
        ...
        for(jj=1;jj<=vpts;jj++) {
          if(theta > E->control.theta_max-gap) {
             if(theta > E->control.theta_max-ramp) {
                temp2 = temp1 + (temp-temp1)*(cos(pow((theta-(E->control.theta_max-ramp))/ramp,3)*3.14159)+1)/2;
                EEta[m][(i-1)*vpts + jj] = temp2;
             }
             else {
                temp2 = temp1 + (temp-temp1)*(cos(pow((theta-(E->control.theta_max-ramp))/ramp,3)*3.14159)+1)/2;
          ...
          if(theta < E->control.theta_min+gap) {
             if(theta < E->control.theta_min+ramp) {
                temp2 = temp1+(temp-temp1)*(cos(pow((E->control.theta_min+ramp-theta)/ramp,3)*3.14159)+1)/2;
                EEta[m][(i-1)*vpts + jj] = temp2;
             }
             else {
                temp2 = temp1+(temp-temp1)*(cos(pow((E->control.theta_min+ramp-theta)/ramp,3)*3.14159)+1)/2;
          ...
          if(phi > E->control.fi_max-gap) {
             if(phi > E->control.fi_max-ramp) {
                temp2 = temp1+(temp-temp1)*(cos(pow((phi-(E->control.fi_max-ramp))/ramp,3)*3.14159)+1)/2;
                if(theta <= E->control.theta_max-0.03 && theta >= E->control.theta_min+0.03)
                   EEta[m][(i-1)*vpts + jj] = temp2;
             }
             else if(theta <= E->control.theta_max-ramp && theta >= E->control.theta_min+ramp){
                temp2 = temp1+(temp-temp1)*(cos(pow((phi-(E->control.fi_max-ramp))/ramp,3)*3.14159)+1)/2;
                if(EEta[m][(i-1)*vpts + jj] > temp2)
                   EEta[m][(i-1)*vpts + jj] = temp2;
             }
          }
          if(phi < E->control.fi_min+gap) {
             if(phi < E->control.fi_min+ramp) {
                temp2 = temp1+(temp-temp1)*(cos(pow((E->control.fi_min+ramp-phi)/ramp,3)*3.14159)+1)/2;
                if(theta <= E->control.theta_max-0.03 && theta >= E->control.theta_min+0.03)
                   EEta[m][(i-1)*vpts + jj] = temp2;
             }
             else if(theta <= E->control.theta_max-ramp && theta >= E->control.theta_min+ramp){
