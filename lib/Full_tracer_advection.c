...
void full_lost_souls(struct All_variables *E)
{
    ...
    for (kk=0;kk<irec[j];kk++) {
        ...
        iel=(E->trace.iget_element)(E,j,E->trace.ntracers[j],-99,x,y,z,theta,phi,rad);
            ...
            if (inum>1 && itry==1)
                {
                    ...
                    ival=(E->trace.iget_element)(E,j,0,-99,x,y,z,theta,phi,rad);
...
int full_iget_element(struct All_variables *E,
                      int j, int tracer, int iprevious_element,
                      double x, double y, double z,
                      double theta, double phi, double rad)
{
    ...
    */
    ...
    if(tracer!=0) {
	fprintf(E->trace.fpt,"tracer,x,y,z,theta,phi,rad %d %.15e %.15e %.15e %.15e %.15e %.15e\n",tracer,
		E->trace.basicq[j][3][tracer],E->trace.basicq[j][4][tracer],E->trace.basicq[j][5][tracer],
		E->trace.basicq[j][0][tracer],E->trace.basicq[j][1][tracer],E->trace.basicq[j][2][tracer]);
	fprintf(E->trace.fpt,"x0,y0,z0,vx,vy,vz %.15e %.15e %.15e %.15e %.15e %.15e\n",
		E->trace.basicq[j][6][tracer],E->trace.basicq[j][7][tracer],E->trace.basicq[j][8][tracer],
		E->trace.basicq[j][9][tracer],E->trace.basicq[j][10][tracer],E->trace.basicq[j][11][tracer]);
	fprintf(E->trace.fpt,"if still in the cap: %d (1 for yes)\n",full_icheck_cap(E,0,x,y,z,rad));
	for(kk=1;kk<=4;kk++)
        {    

            fprintf(E->trace.fpt,"xcap[%d]=%.15e\n",kk,E->trace.xcap[0][kk]);
            fprintf(E->trace.fpt,"ycap[%d]=%.15e\n",kk,E->trace.ycap[0][kk]);
            fprintf(E->trace.fpt,"zcap[%d]=%.15e\n",kk,E->trace.zcap[0][kk]);
            fprintf(E->trace.fpt,"theta_cap[%d]=%.15e\n",kk,E->trace.theta_cap[0][kk]);
            fprintf(E->trace.fpt,"phi_cap[%d]=%.15e\n",kk,E->trace.phi_cap[0][kk]);
            fprintf(E->trace.fpt,"cos_theta[%d]=%.15e\n",kk,E->trace.cos_theta[0][kk]);
            fprintf(E->trace.fpt,"sin_theta[%d]=%.15e\n",kk,E->trace.sin_theta[0][kk]);
            fprintf(E->trace.fpt,"cos_phi[%d]=%.15e\n",kk,E->trace.cos_phi[0][kk]);
            fprintf(E->trace.fpt,"sin_phi[%d]=%.15e\n",kk,E->trace.sin_phi[0][kk]);
        }
    }
    fflush(E->trace.fpt);
    return -1;
