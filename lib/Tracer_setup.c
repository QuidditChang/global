...
void count_tracers_of_flavors(struct All_variables *E)
{
    ...
    const int oc=1,obc=2,ol=3,opl=4,oecl=5,weakarc=6,cuc=7,clc=8,cul=9,cml=10,cll=11,cratuc=12,cratlc=13,cratul=14,cratml=15,cratll=16,cecl=17,chem=18,track=19;
    double PI=3.1415926;
	...
	if(E->parallel.me==0)
	   ...
	   /*if(E->monitor.solution_cycles==0 && age1==0.0 && rad>0.996) {
	    ...
	    /* add a chemical layer above CMB */
            ...
            /* add a tracer in CMB */
            if(rad<E->sphere.ri && E->monitor.solution_cycles==0)
                 E->trace.extraq[j][0][kk]=track;
