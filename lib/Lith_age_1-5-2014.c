...
void lith_age_construct_tic(struct All_variables *E)
{
  ...
  fi = 0.5-0.0*(3.14159/(11.1*180.0))*(E->control.start_age-age);
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++)
	...
	for(k=1;k<=noz;k++)  {
	  ...
	  phi = E->sx[m][2][node];
          ...
          /*if(phi>fi+0.0 && phi<=fi+1)
             assim_depth = E->control.lith_age_depth*0.2; */
	  //if(phi>fi+0.05 && phi<fi+0.08)
	    // assim_depth = E->control.lith_age_depth*cos((fi-phi+0.08)*1.5708/0.03);
...
void lith_age_temperature_bound_adj(struct All_variables *E, int lv)
{
  ...
  float find_age_in_MY(),age,fi,phi,v_trench;
  FILE *fp1;
...
all three get set to true. CPC 6/20/00 */
  ...
  if (E->control.lith_age_time) {
    ...
    //fi = 4.5+v_trench*(3.14159/(11.1*180.0))*(100.0-age);
    fi = 0.5+2*v_trench*(3.14159/(11.1*180.0))*(E->control.start_age-age);
    //fi += v_trench*(3.14159/(11.1*180.0))*(E->control.start_age-age);

    assim_depth = E->control.lith_age_depth;
    if(E->convection.tic_method == -1) assim_depth = E->control.lith_age_depth/2.0; 

   /* if(E->monitor.solution_cycles>1)
	assim_depth = E->control.lith_age_depth/10.0; */
    //else
//	assim_depth = 0.002;
    ...
    if(lv==E->mesh.gridmax)
      for(j=1;j<=E->sphere.caps_per_proc;j++)
		...
		/*if(E->monitor.solution_cycles>1 && phi>0.28)
		...
		//if(E->age_t[nodeg]>180.0/E->data.scalet) assim_depth = E->control.lith_age_depth/10.0;

	  	//if(E->sx[j][3][node]>=E->sphere.ro-E->control.lith_age_depth)
		if(E->sx[j][3][node]>=E->sphere.ro-assim_depth) {
	  	   /* if closer than (lith_age_depth) from top */
	  	    if(E->age_t[nodeg]>0.01/E->data.scalet) { // && phi<fi-0.02) { // || E->sx[j][3][node]>=E->sphere.ro-0.001) {
	  	        E->node[j][node]=E->node[j][node] | TBX;
	  	        E->node[j][node]=E->node[j][node] & (~FBX);
	  	        E->node[j][node]=E->node[j][node] | TBY;
	  	        E->node[j][node]=E->node[j][node] & (~FBY);
...
//		temp = (E->sphere.ro-r1) *0.5 /sqrt(E->age_t[nodeg]);
		if(E->age_t[nodeg]>0.01/E->data.scalet)
                    /*if(E->sx[m][3][node]>=E->sphere.ro-E->control.lith_age_depth*1/2.0)
                      temp = (E->sphere.ro-r1) *0.5 /sqrt(E->age_t[nodeg])/2;
                    else
                     temp = (E->sphere.ro-r1) *0.5 /sqrt(E->age_t[nodeg])/1.0; */
                    temp = (E->sphere.ro-r1) *0.5 /sqrt(E->age_t[nodeg])/5;
                else
                    temp = 10.0;
		t0 = E->control.lith_age_mantle_temp * erf(temp);
...
{
  float depth, daf, assimilate_new_temp,assim_depth,fi,phi,temp1;
  ...
  for(j=1;j<=E->sphere.caps_per_proc;j++)
      for(i=1;i<=noy;i++)
		...
		if(E->age_t[nodeg]>0.01/E->data.scalet && phi<fi-0.02) {
		...
		//if(E->age_t[nodeg]>0.0 && E->age_t[nodeg]<180.0/E->data.scalet && phi>fi-0.01)  {
		/*if(E->age_t[nodeg]>0.0 && E->age_t[nodeg]<180.0/E->data.scalet && phi<fi+0.02) {
		    if(depth <= 0.003 && phi>fi-0.01) {
                          daf = 1.0*depth/0.003;
                          E->T[j][node] = daf*E->T[j][node] + (1.0-daf)*assimilate_new_temp;
                    }
		    else {
                        if(phi>fi-0.02)
                           temp1 = 0.003 + E->control.lith_age_depth*((fi-0.01)-phi)/0.01;
                        else
