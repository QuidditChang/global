...
/* CBF variants: surface and CMB heat flux via Consistent Boundary Flux method */
extern void heat_flux_CBF_surf(struct All_variables *);
extern void heat_flux_CBF_cmb(struct All_variables *);
...
void h5output_surf_botm(struct All_variables *E, int cycles)
{
    ...
    if((E->output.write_q_files == 0) || (cycles == 0) ||
       (cycles % E->output.write_q_files)!=0) {
        heat_flux(E);
        if (E->output.use_cbf_shflux)  heat_flux_CBF_surf(E);
        if (E->output.use_cbf_bhflux)  heat_flux_CBF_cmb(E);
    }
    ...
    if (E->output.surf == 1)
    {
        ...
        h5create_field(surf_group, E->hdf5.scalar2d, "heatflux",
                       "top surface heatflux (legacy extrapolation)");
        if (E->output.use_cbf_shflux)
            h5create_field(surf_group, E->hdf5.scalar2d, "heatflux_cbf",
                           "top surface heatflux (CBF method)");
        ...
        for(i = 0; i < mx; i++)
            for(j = 0; j < my; j++) {
                n = k + i*nz + j*nz*nx;
                scalar->data[j + i*my] = E->slice.shflux[1][n+1];
            }
        dataset = H5Dopen(file_id, "/surf/heatflux");
        status = h5write_field(dataset, scalar, 0, (pz == nprocz-1));
        status = H5Dclose(dataset);

        /* CBF heatflux (written only when use_cbf_shflux=1) */
        if (E->output.use_cbf_shflux) {
            for(i = 0; i < mx; i++)
                for(j = 0; j < my; j++) {
                    n = k + i*nz + j*nz*nx;
                    scalar->data[j + i*my] = E->slice.cbf_shflux[1][n+1];
                }
            dataset = H5Dopen(file_id, "/surf/heatflux_cbf");
            status = h5write_field(dataset, scalar, 0, (pz == nprocz-1));
            status = H5Dclose(dataset);
        }
    ...
    if (E->output.botm == 1)
    {
        ...
        h5create_field(botm_group, E->hdf5.scalar2d, "heatflux",
                       "bottom surface heatflux (legacy extrapolation)");
        if (E->output.use_cbf_bhflux)
            h5create_field(botm_group, E->hdf5.scalar2d, "heatflux_cbf",
                           "bottom surface heatflux (CBF method)");
        ...
        for(i = 0; i < mx; i++)
            for(j = 0; j < my; j++) {
                n = k + i*nz + j*nz*nx;
                scalar->data[j + i*my] = E->slice.bhflux[1][n+1];
            }
        dataset = H5Dopen(file_id, "/botm/heatflux");
        status = h5write_field(dataset, scalar, 0, (pz == 0));
        status = H5Dclose(dataset);

        /* CBF heatflux (written only when use_cbf_bhflux=1) */
        if (E->output.use_cbf_bhflux) {
            for(i = 0; i < mx; i++)
                for(j = 0; j < my; j++) {
                    n = k + i*nz + j*nz*nx;
                    scalar->data[j + i*my] = E->slice.cbf_bhflux[1][n+1];
                }
            dataset = H5Dopen(file_id, "/botm/heatflux_cbf");
            status = h5write_field(dataset, scalar, 0, (pz == 0));
            status = H5Dclose(dataset);
        }
