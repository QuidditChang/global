	...
	for(m=1;m<=E->sphere.caps_per_proc;m++)
		...
		flag2 = 0;
		    ...
		    /* creat a smooth radial visc. profile */
                    ...
                    else if(r<0.97 && r>0.93) {
                        tempa = E->viscosity.N0[1] + (E->viscosity.N0[2]-E->viscosity.N0[1])*(cos((r-0.93)/0.04*3.14159)+1)/2;
                        viscE = E->viscosity.E[1] + (E->viscosity.E[2]-E->viscosity.E[1])*(cos((r-0.93)/0.04*3.14159)+1)/2;
                        viscT = E->viscosity.T[1] + (E->viscosity.T[2]-E->viscosity.T[1])*(cos((r-0.93)/0.04*3.14159)+1)/2;
                    }
			...
			if(rheo_trick == 1) { //a strong slab in the upper mantle as a stress guide
					...
					if (E->sx[m][2][node]>fi && E->sx[m][2][node]<fi+lon_inc1+0.03 && r>0.99){
						...
						//temp2 = 0.0001+(1.0-(1*cos((pow(temp1/dist1,1)*1.57))));
					    ...
					    if(phi-(fi+dip*(1.0-r))>0.0){
                                                if(temp1>=0.0 && temp1<dist1 && r>d_wdg && flag2==0 && r<0.999) { // && r<1.0
