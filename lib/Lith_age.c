...
void lith_age_construct_tic(struct All_variables *E)
{
  ...
  int nox, noy, noz, gnox, gnoy, gnoz,num4,num5,num6;
  ...
  double PI=3.1415926;
  double inputdata,tmp1,tmp2,*geomy4,*geomx4,*geomy5,*geomx5,*geomy6,*geomx6;
  char pb[250];
  FILE *fp,*flgeo;
  noy=E->lmesh.noy;
  nox=E->lmesh.nox;
  ...
  gnoy=E->mesh.noy;
  gnoz=E->mesh.noz;
  geomx4=(double *) malloc(250*sizeof(double));
  geomy4=(double *) malloc(250*sizeof(double));
  geomx5=(double *) malloc(250*sizeof(double));
  geomy5=(double *) malloc(250*sizeof(double));
  geomx6=(double *) malloc(250*sizeof(double));
  geomy6=(double *) malloc(250*sizeof(double));

  num4=num5=num6=0;
  sprintf(pb,"Pacific_pile.xy");
  fprintf(E->fp,"Read Pacific Pile: %s\n",pb);
  fprintf(stderr,"Read Pacific Pile: %s\n",pb);
  fflush(E->fp);
  fflush(stderr);
  flgeo=fopen(pb,"r");
  while(fscanf(flgeo,"%lf %lf",&tmp1,&tmp2)>0) {
     ...
     if(tmp1<0.0) tmp1 += 360.0;
         geomx4[num4]=tmp1/180*PI;
         geomy4[num4]=PI/2.0-tmp2/180*PI;}
     fclose(flgeo);
              
     if(num4<3) {
         fprintf(E->fp,"Read in pile polygon error, less than three points in file");
         exit(8);}
      geomx4[num4+1]=geomx4[1];
      geomy4[num4+1]=geomy4[1];

   sprintf(pb,"Africa_pile1.xy");
   fprintf(E->fp,"Read Africa Pile1: %s\n",pb);
   fprintf(stderr,"Read Africa Pile1: %s\n",pb);
   fflush(E->fp);
   fflush(stderr);
   flgeo=fopen(pb,"r");
   while(fscanf(flgeo,"%lf %lf",&tmp1,&tmp2)>0) {
      num5++;
      if(tmp1<0.0) tmp1 += 360.0;
          geomx5[num5]=tmp1/180*PI;
          geomy5[num5]=PI/2.0-tmp2/180*PI; }
      fclose(flgeo);
      if(num5<3) {
          fprintf(E->fp,"Read in pile polygon error, less than three points in file");
          exit(8);
          }
      geomx5[num5+1]=geomx5[1];
      geomy5[num5+1]=geomy5[1];

   sprintf(pb,"Africa_pile2.xy");
   fprintf(E->fp,"Read Africa Pile2: %s\n",pb);
   fprintf(stderr,"Read Africa Pile2: %s\n",pb);
   fflush(E->fp);
   fflush(stderr);
   flgeo=fopen(pb,"r");
   while(fscanf(flgeo,"%lf %lf",&tmp1,&tmp2)>0) {
      num6++;
      if(tmp1<0.0) tmp1 += 360.0;
          geomx6[num6]=tmp1/180*PI;
          geomy6[num6]=PI/2.0-tmp2/180*PI; }
      fclose(flgeo);
      if(num6<3) {
          fprintf(E->fp,"Read in pile polygon error, less than three points in file");
          exit(8);
          }
      geomx6[num6+1]=geomx6[1];
      geomy6[num6+1]=geomy6[1];
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++)
	...
	/* add a chemical layer above CMB */
       //(isptin(geomx4,geomy4,num4,phi,theta)!=0 || isptin(geomx5,geomy5,num5,phi,theta)!=0) //
	else if( r1 <= E->sphere.ri+3.2*E->control.lith_age_depth && (isptin(geomx4,geomy4,num4,phi,theta)!=0 || isptin(geomx5,geomy5,num5,phi,theta)!=0 || isptin(geomx6,geomy6,num6,phi,theta)!=0)) {
	    temp = (r1 - E->sphere.ri) *0.5 /sqrt(2000./E->data.scalet);
