...
void lith_age_construct_tic(struct All_variables *E)
{
  ...
  double left_bd, right_bd, lon_rad, colat_rad;
  noy=E->lmesh.noy;
  nox=E->lmesh.nox;
  noz=E->lmesh.noz;
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++)
	...
	else if(r1 <= E->sphere.ri+1.00*E->control.lith_age_depth) {
	...
	lon_rad = E->sx[m][2][node];
	colat_rad = E->sx[m][1][node];
	left_bd = 270*PI/180;
	right_bd = 330*PI/180;
	...
	else if (flag_depth2<0.0 && flag_depth2>-0.05 && r1>=0.93 && r1<0.95 && lon_rad<5.759586531581 && lon_rad>4.188790204786 && colat_rad<2.9146998508305 && colat_rad>0.349065850398659){
		temp1 = -0.05;
		temp2 = -0.03;
		if(flag_depth2<=temp2 && flag_depth2>=temp1){
			dist2=0.05+flag_depth2;
			temp=dist2*0.5/sqrt(50.0/E->data.scalet);
                    	E->T[m][node]=E->control.lith_age_mantle_temp * erf(temp);
		}
		
	   }
	else if (flag_depth2<0.0 && flag_depth2>-0.05 && r1>=0.92 && r1<0.95 && colat_rad<3.1415926536 && colat_rad>2.79252680319){
		temp1 = -0.05;
		temp2 = -0.03;
		if(flag_depth2<=temp2 && flag_depth2>=temp1){
			dist2=0.05+flag_depth2;
