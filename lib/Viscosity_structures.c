		    ...
		    if(E->T[m][node]<=-0.01) {
			   ...
			   /*if(r>0.97 && fabs(E->flag_depth2[nodeg])*1000.0>200) { // && E->age_t[nodeg]>380.0/E->data.scalet) {
			    ...
			    if(temp < one)  /* enhance the weak viscosity of plume */
			       EEta[m][ (i-1)*vpts + jj ] = tempa*exp(15/(temp+viscT) - 15/(one+viscT) );
			      //    if(r<0.99)
                            //       EEta[m][ (i-1)*vpts + jj ] = tempa*exp(5.0/(temp+viscT) - 5.0/(one+viscT) );
			   //    else
                           //        EEta[m][ (i-1)*vpts + jj ] = tempa*exp(8.0/(temp+viscT) - 8.0/(one+viscT) );
			   //}
			   else
