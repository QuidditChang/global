...
void output_surf_botm(struct All_variables *E, int cycles)
{
  ...
  if (E->output.surf && (E->parallel.me_loc[3]==E->parallel.nprocz-1)) {
    ...
    if (E->output.use_cbf_shflux) {
      sprintf(output_file,"%s.surf_cbf.%d.%d", E->control.data_file,
              E->parallel.me, cycles);
      fp2 = output_open(output_file, "w");
      for(j=1;j<=E->sphere.caps_per_proc;j++)  {
          if(E->control.pseudo_free_surf)
          ...
          for(i=1;i<=E->lmesh.nsf;i++)   {
              s = i*E->lmesh.noz;
              fprintf(fp2,"%.4e %.4e %.4e %.4e\n",topo[i],E->slice.cbf_shflux[j][i],E->sphere.cap[j].V[1][s],E->sphere.cap[j].V[2][s]);
          }
      }
      fclose(fp2);
    }
  ...
  if (E->output.botm && (E->parallel.me_loc[3]==0)) {
    ...
    if (E->output.use_cbf_bhflux) {
      sprintf(output_file,"%s.botm_cbf.%d.%d", E->control.data_file,
              E->parallel.me, cycles);
      fp2 = output_open(output_file, "w");
      for(j=1;j<=E->sphere.caps_per_proc;j++)  {
        fprintf(fp2,"%3d %7d\n",j,E->lmesh.nsf);
        for(i=1;i<=E->lmesh.nsf;i++)  {
          s = (i-1)*E->lmesh.noz + 1;
          fprintf(fp2,"%.4e %.4e %.4e %.4e\n",E->slice.tpgb[j][i],E->slice.cbf_bhflux[j][i],E->sphere.cap[j].V[1][s],E->sphere.cap[j].V[2][s]);
        }
      }
      fclose(fp2);
    }
...
void output_mat(struct All_variables *E)
{
  ...
  float *eedot;
  double t1,t2;
  eedot = (float *) malloc((2+E->lmesh.nel)*sizeof(float));
  ...
  for(m=1;m<=E->sphere.caps_per_proc;m++)  {
    for(el=1;el<=E->lmesh.nel;el++)
      E->VIP[m][el] =1.0 / ( eedot[el]/t2 * 10 + 1.0);
  }
  /* End of addition */
  
  sprintf(output_file,"%s.mat.%d", E->control.data_file,E->parallel.me);

  fprintf(E->fp,"writing mat file\n",output_file);
