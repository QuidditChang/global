...
//	      temp = (E->sphere.ro-r1) *0.5 /sqrt(E->age_t[nodeg]);
	      ...
	      if(E->age_t[nodeg]>0.0/E->data.scalet) {//Lijun: define continent to be 0 age
...
void lith_age_temperature_bound_adj(struct All_variables *E, int lv)
{
  int j,node,nno;
  int ii,jj,kk,nodeg;
  float ttt2,ttt3,fff2,fff3,assim_depth,v_trench;
  float find_age_in_MY(),age,fi,phi;
  FILE *fp1;
...
all three get set to true. CPC 6/20/00 */
  ...
  if (E->control.lith_age_time) {
    ...
    if(E->parallel.me==0) {
        fprintf(E->fp,"Lith_age.c: v_trench=%f\n",v_trench);
    }
...
//	assim_depth = 0.002;
    ...
    if(lv==E->mesh.gridmax)
      for(j=1;j<=E->sphere.caps_per_proc;j++)
		...
		/*if(phi>fi-0.02 && phi<=fi) { // && E->monitor.solution_cycles>1)
		    assim_depth = 0.0 + E->control.lith_age_depth*(1.0-cos((fi-phi)/0.02*1.57));
		}*/

	  	if(E->sx[j][3][node]>=E->sphere.ro-E->control.lith_age_depth) {
		//if(E->sx[j][3][node]>=E->sphere.ro-assim_depth) {
	  	   /* if closer than (lith_age_depth) from top */
	  	    if(E->age_t[nodeg]>1.0/E->data.scalet && E->sx[j][3][node]>=E->sphere.ro-assim_depth) {
	  	        E->node[j][node]=E->node[j][node] | TBX;
	  	        E->node[j][node]=E->node[j][node] & (~FBX);
	  	        E->node[j][node]=E->node[j][node] | TBY;
	  	        E->node[j][node]=E->node[j][node] & (~FBY);
...
void lith_age_conform_tbc(struct All_variables *E)
{
  ...
  if(E->monitor.solution_cycles>1 && E->control.temperature_bound_adj) {
	...
	for(j=1;j<=nox;j++)
	  for(k=1;k<=noz;k++)  {
		...
		/* if < (width) from x bounds AND (depth) from top */
		if(E->age_t[nodeg]>1.0/E->data.scalet) //Lijun define continent to be 0 age with T_sfc=1.0
		    temp = (E->sphere.ro-r1) *0.5 /sqrt(E->age_t[nodeg])/2.0;
		else
		    temp = 10.0;
  ...
  //if(E->monitor.solution_cycles>1) E->control.lith_age_depth = 0.003;
...
//		temp = (E->sphere.ro-r1) *0.5 /sqrt(E->age_t[nodeg]);
		if(E->age_t[nodeg]>0.0/E->data.scalet)
                    temp = (E->sphere.ro-r1) *0.5 /sqrt(E->age_t[nodeg])/2.0;
                else
                    temp = 10.0;
		t0 = E->control.lith_age_mantle_temp * erf(temp);
...
{
  float depth, daf, assimilate_new_temp,assim_depth,fi,phi;
  ...
  for(j=1;j<=E->sphere.caps_per_proc;j++)
      for(i=1;i<=noy;i++)
		...
		if(E->age_t[nodeg]>1.0/E->data.scalet) {
