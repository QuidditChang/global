...
struct SLICE {    /* horizontally sliced data, including topography */
    ...
    float *shflux[NCS];
    float *bhflux[NCS];
    /* CBF (Consistent Boundary Flux) variants: per-node flux maps, no extrapolation.
     * shflux / bhflux use legacy Gauss-point extrapolation (backward compatible).
     * cbf_shflux = surface (top)   heat flux via CBF (Gresho et al. 1987).
     * cbf_bhflux = CMB    (bottom) heat flux via CBF (Gresho et al. 1987). */
    float *cbf_shflux[NCS];
    float *cbf_bhflux[NCS];
    float *divg[NCS];
    float *vort[NCS];
    float *freesurf[NCS];
  };
...
struct CONTROL {
  ...
  int nrlayer[20],rlayers;
    ...
    char flag_depth_new_file[1000];
    char flag_depth_file[1000];
    char tf_file[1000];
...
struct Output {
  ...
  int use_cbf_shflux; /* 1 = CBF method for surface heat flux;  0 = legacy extrapolation */
  int use_cbf_bhflux; /* 1 = CBF method for CMB (bottom) heat flux; 0 = legacy extrapolation */
};
...
#include "advection.h"
    ...
    /* Lijun define a time-dependent viscosity component */
    float *oldVI[MAX_LEVELS][NCS],*oldEVI[MAX_LEVELS][NCS];
    ...
    float *flag_depth1;
    float *flag_depth2;
    float *new_flag_depth;
    float *tf_depth;
    //float *craton;
    int trench_visit_age;
