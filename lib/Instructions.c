...
void read_initial_settings(struct All_variables *E)
{
  ...
  */
  ...
  /* 0 = legacy Gauss-point extrapolation for both boundaries (backward compatible).
   * 1 = CBF method (Gresho et al. 1987) for both surface and CMB:
   *     the energy-equation residual is integrated over the single boundary-adjacent
   *     element layer and divided by the lumped boundary mass matrix, giving
   *     per-node flux maps that satisfy the global energy balance exactly. */
  input_int("use_cbf_shflux",&(E->output.use_cbf_shflux),"0",m);
  input_int("use_cbf_bhflux",&(E->output.use_cbf_bhflux),"0",m);
...

{
 ...
 for (j=1;j<=E->sphere.caps_per_proc;j++)  {
  ...
  /* CBF per-node flux arrays, same layout as shflux/bhflux (nsf surface nodes each) */
  E->slice.cbf_shflux[j] = (float *)malloc((nsf+2)*sizeof(float));
  E->slice.cbf_bhflux[j] = (float *)malloc((nsf+2)*sizeof(float));
