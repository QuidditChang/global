  ...
  do {
    ...
    do {
        ...
        if (E->advection.ADVECTION) {
            ...
            for(psc_pass=0;psc_pass<E->advection.temp_iterations;psc_pass++)   {
                pg_solver(E,E->T,E->Tdot,DTdot,E->convection.heat_sources,diff,1,E->node);
                corrector(E,E->T,E->Tdot,DTdot);
                temperatures_conform_bcs(E);
 //               if(E->sphere.caps==1)  apply_smooth_sideTbc(E);
            }
        ...
        if (fabs(E->monitor.T_interior/T_interior1) > E->monitor.T_maxvaried && flag==0) {
              ...
              if( E->advection.timestep <= 0.005*E->advection.fixed_timestep || flag == 1) {
                ...
                do {
                    ...
                    i_smth++;
                } while (E->monitor.T_interior/T_interior1 > 1.01 && E->monitor.T_interior/T_interior1<ratio && i_smth < 50);
                      ...
                      double diff, int bc, unsigned int **FLAGS)
{
    ...
    int el,e,a,i,a1,m;
    double Eres[9],rtf[4][9],t1,t2;  /* correction to the (scalar) Tdot field */
    float VV[4][9];
    ...
    for(i=1;i<=vpts;i++) {
      ...
      for(phase_index=0;phase_index<PHASE_TRANSITIONS;phase_index++) {
        ...
        if(phase->entropy_jump == 0.0)
          continue;
        /* form_rtf_bc stores inverse radius in rtf[3]. */
