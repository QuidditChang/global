     ...
     float **evisc,**visc;
{
    ...
    if(E->control.tracer && E->monitor.solution_cycles > 0 && propogate)
      read_tracer_visc(E,E->oldEVI[E->mesh.levmax]);
			...
			if(rheo_trick == 1) { //a strong slab in the upper mantle as a stress guide
					...
					if (E->sx[m][2][node]>fi && E->sx[m][2][node]<fi+lon_inc1+0.03 && r>0.99){
					    ...
					    //	fprintf(E->fp,"fi=%f\n",fi);
                                            ...
                                            dist1 = 0.03;
                                            if(temp1>-dist1 && temp1<0.03 && r>0.98 && phi<fi+0.02) { // && theta<=lat_max+0.015 && theta>=lat_min+0.005) {
					    ...
					    temp2 = 5.0;
                                            ...
                                            dist1 = 0.03;
                                            if(temp1>0.0-dist1 && temp1<0.03 && r>0.985 && phi>fi-0.03){
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)  {
	  ...
	  eta_p = tau/(2.0 * eedot[e] + 1e-7); /* + E->viscosity.pdepv_offset; */
	   ...
	   /* Lijun change to flavor function; turned on only below oceanic MOHO */
          if (E->control.tracer && zzz > 0.003 && zzz < 0.04) {
 ...
 *          element */
        ...
        for(jj = 1; jj <= vpts; jj++) {
            ...
            /* multiply the viscosity with this prefactor */
            if(E->sx[m][3][E->ien[m][i].node[1]]>0.96 && E->monitor.solution_cycles == 0) {
            ...
            for(p=1;p<E->composition.ncomp;p++) {
               if(E->trace.ntracer_flavor[m][p][i]>0.0 && E->monitor.solution_cycles>0) { // && E->sx[m][3][E->ien[m][i].node[1]]>0.96) {
...
static void record_tracer_visc(E,EEta)
      struct All_variables *E;
      float **EEta;
{
      ...
      for(kk=1;kk<=E->trace.ntracers[m];kk++) { /* loop through all tracers in serial */
          ...
          if(e > 0) {
             ...
             for(jj=1;jj<=vpts;jj++) {
                ...
                visc_tracer += EEta[m][(e-1)*vpts+jj]*weight[jj];
             }
...
static void read_tracer_visc(E,oldEEta)
      struct All_variables *E;
      float **oldEEta;
{
   int m,e,kk,jj;
         ...
         if(el_count[e]>0) {
            ...
            for(jj=1;jj<=vpts;jj++) {
                if(oldEEta[m][(e-1)*vpts+jj] == 0.0)
                   oldEEta[m][(e-1)*vpts+jj] = visc_mean;
            } /* end of jj loop */
      } /* end of if statement */
