  ...
  case 0:
	...
	for(j=1;j<=nox;j++)
	  for(k=1;k<=noz;k++) {
	    ...
	    r1=E->sx[m][3][node];
	    f1=E->sx[m][2][node] - E->control.fi_min;
	    //E->T[m][node] = E->control.TBCbotval - (E->control.TBCtopval + E->control.TBCbotval)*(r1 - E->sphere.ri)/(E->sphere.ro - E->sphere.ri);
	    if(r1>0.95 && f1<0.5) {
	    	//E->T[m][node] = E->control.TBCbotval - (E->control.TBCtopval + E->control.TBCbotval)*(r1 - 0.95)/(E->sphere.ro - 0.95);
	    	E->T[m][node] = E->control.lith_age_mantle_temp - (E->control.lith_age_mantle_temp - E->control.TBCtopval)*(r1 - 0.95)/(E->sphere.ro - 0.95);
	    }
	    else if(r1>0.96 && f1>=0.5) {
	    	E->T[m][node] = E->control.lith_age_mantle_temp - (E->control.lith_age_mantle_temp - E->control.TBCtopval)*(r1 - 0.96)/(E->sphere.ro - 0.96);
	    }
	    else {
	    ...
	    if(r1<=0.59)
		E->T[m][node] = E->control.lith_age_mantle_temp + (E->control.TBCbotval - E->control.lith_age_mantle_temp)*(0.59 - r1)/(0.59 - E->sphere.ri);
	  }
  ...
  case 1:
      ...
      for(m=1;m<=E->sphere.caps_per_proc;m++)
          for(i=1;i<=noy;i++)
		      ...
		      if(rad < 0.99 && rad >= 0.95){ 
			//E->T[m][node] = E->control.lith_age_mantle_temp + (1.0-E->control.lith_age_mantle_temp)*cos((rad-0.95)/(0.99-0.95)*3.14159/2.0);
			E->T[m][node] = E->control.TBCbotval - (E->control.TBCtopval + E->control.TBCbotval)*(rad - 0.95)/(E->sphere.ro - 0.95);
		      }
		      if(rad < 0.95) {
			E->T[m][node] = E->control.TBCbotval;
		      }
