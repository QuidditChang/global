  ...
  case 1:
      ...
      for(m=1;m<=E->sphere.caps_per_proc;m++)
          for(i=1;i<=noy;i++)
		      ...
		      E->T[m][node] = 0.8*erf(temp);
		      /* Lijun: add in a hot layer btwn 200 & 400 km depth */
		      if(r1<=0.968 && r1>=0.937)
			//E->T[m][node] =  E->control.lith_age_mantle_temp + (E->control.TBCbotval-E->control.lith_age_mantle_temp)*cos((r1-0.9525)/(0.9525-0.937)*3.14159/2.0);
			E->T[m][node] = 0.8 + (1.0-0.8)*cos((r1-0.9525)/(0.9525-0.937)*3.14159/2.0);
                  }
