			...
			if(rheo_trick == 1) { //a strong slab in the upper mantle as a stress guide
					...
					if (E->sx[m][2][node]>fi && E->sx[m][2][node]<fi+lon_inc1+0.03 && r>0.99){
					    ...
					    //	fprintf(E->fp,"fi=%f\n",fi);
                                            ...
                                            dist1 = 0.03;
                                            if(temp1>-dist1 && temp1<0.03 && r>0.98) { // && theta<=lat_max+0.015 && theta>=lat_min+0.005) {
     ...
     float **EEta;
{
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)  {
	  ...
	  /*if(E->parallel.me==0 && E->monitor.solution_cycles > 0) {
	    fprintf(E->fp,"eta_p=%f,visc_now=%f\n",eta_p,EEta[m][ (e-1)*vpts + jj ]);
	    fflush(E->fp);
          }*/
	   ...
	   /* Lijun change to single flavor function */
          if (E->control.tracer && zzz < 0.02) {
	      if(E->trace.ntracer_flavor[m][0][e] > 0.0) // number of flavor 1 tracers per elem.
