...
static void visc_from_tracers();
static void viscosity_filter();
static void weak_plate_boundary_from_velocities();
...
void viscosity_system_input(struct All_variables *E)
{
    ...
    input_boolean("GDEPV",&(E->viscosity.GDEPV),"off",m);
    if (E->viscosity.GDEPV)
        E->viscosity.gdepv_visited = 0;
    ...
    if(E->viscosity.PDEPV || E->viscosity.SDEPV || E->viscosity.GDEPV)
          ...
          for(kkk=1;kkk<=E->lmesh.noy;kkk++)
            for(jjj=1;jjj<=E->lmesh.nox;jjj++){
                ...
                for(iii=E->lmesh.noz;iii>=1;iii--)  {
                    ...
                    else if(r<0.97 && r>0.94) {
                        tempa = E->viscosity.N0[1] + (E->viscosity.N0[2]-E->viscosity.N0[1])*(cos((r-0.94)/0.03*3.14159)+1)/2;
                        viscE = E->viscosity.E[1] + (E->viscosity.E[2]-E->viscosity.E[1])*(cos((r-0.94)/0.03*3.14159)+1)/2;
                        viscT = E->viscosity.T[1] + (E->viscosity.T[2]-E->viscosity.T[1])*(cos((r-0.94)/0.03*3.14159)+1)/2;
                    }
                    else if(r<0.91 && r>0.71) {
                        tempa = E->viscosity.N0[3] + (E->viscosity.N0[2]-E->viscosity.N0[3])*(fabs(0.71-r)/0.2);
                        viscE = E->viscosity.E[3] + (E->viscosity.E[2]-E->viscosity.E[3])*(fabs(0.71-r)/0.2);
                        viscT = E->viscosity.T[3] + (E->viscosity.T[2]-E->viscosity.T[3])*(fabs(0.71-r)/0.2);
                    }
                    else {*/
                        tempa = E->viscosity.N0[l-1];
		...
		flag2 = 0;
			...
			if(rheo_trick == 1) { //a strong slab in the upper mantle as a stress guide
					...
					if (E->sx[m][2][node]>fi && E->sx[m][2][node]<fi+lon_inc1+0.03 && r>0.99){
					    ...
					    temp2 = 5.0; 
					    fi = 0.5+v_trench*(3.14159/(11.1*180.0))*(200.0-age);
					    ...
					    tempa = E->viscosity.N0[l-1];
                        		    ...
                        		    viscT = E->viscosity.T[l-1];
                                            ...
                                            /*if(E->control.mat_control==0) {
                                                    temp1 = 1000*tempa; // *(1.1-cos((r-0.9)/0.1*1.57));
                                                    if(EEta[m][(i-1)*vpts+jj]>temp1 && r>0.92 && r<1.1)
                                                      if(phi>fi-0.2 && phi<fi+0.4){
                                                        if(phi>fi || phi<fi && r<0.99)
                                                           EEta[m][(i-1)*vpts+jj]= temp1; //50*tempa*(1.1-cos((r-0.9)/0.1*1.57));
                                                           //if(r>0.99 && phi<fi+0.03)
                                                            //EEta[m][(i-1)*vpts+jj] *= 0.3*(1.0-cos(pow((1.0-r)/0.01,2)*1.57));
                                                      }
					    ...
					    //	fprintf(E->fp,"fi=%f\n",fi);
                                            ...
                                            dist1 = 0.03;
                                            if(temp1>-dist1 && temp1<0.03 && r>0.986 && phi<fi+0.02) { // && theta<=lat_max+0.015 && theta>=lat_min+0.005) {
					    ...
					    temp2 = 3.0;
                                            ...
                                            dist1 = 0.03;
                                            if(temp1>0.0-dist1 && temp1<dist1 && r>0.99 && phi>fi-dist1){
					    ...
					    if(phi-(fi+dip*(1.0-r))>0.0){
                                                if(temp1>=0.0 && temp1<dist1 && r>d_wdg && r<1.0 && flag2==0) {
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)  {
      ...
      if(E->viscosity.sdepv_visited){
        ...
        //if(E->parallel.me == 0) {
           ...
           fprintf(E->fp,"EEta[10000] = %f\n", EEta[m][10000]);
           fprintf(E->fp,"nel = %d  ", E->lmesh.nel);
          // fprintf(E->fp,"propogate = %d\n", propogate);
		...
		t1 = scale*pow(EEta[m][(e-1)*vpts+jj],exponent1)*adj_factor;
                //fprintf(E->fp,"t1=%f ",t1);
                //fprintf(E->fp,"EEta[m][%d]=%f ", (e-1)*vpts+jj, EEta[m][(e-1)*vpts+jj]);
                //fprintf(E->fp,"eedot[%d]=%f ", e, eedot[e]);
     ...
     float **EEta;
{
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)  {
	  ...
	  eta_p = tau/(2.0 * eedot[e] + 1e-7); /* + E->viscosity.pdepv_offset; */
          ...
          /*if(eedot[e]>10.0)
             eta_p = EEta[m][ (e-1)*vpts + jj ]/(eedot[e] + 1e-7);
	  else
	     eta_p = EEta[m][ (e-1)*vpts + jj ];*/
	   ...
	   /* Lijun change to flavor function; turned on only below oceanic MOHO */
          if (zzz > 0.005) {
              //for(p=1; p<E->composition.ncomp; p++) {  /* PDEPV not apply to background mantle (composition 0) */
                // if(E->trace.ntracer_flavor[m][p][e] > 0.0)
                    EEta[m][(e-1)*vpts + jj] = eta_new;
                    /*EEta[m][(e-1)*vpts + jj] += (eta_new-EEta[m][(e-1)*vpts + jj])*(E->trace.ntracer_flavor[m][0][e]/4.0);*/
              //}
          }
     ...
     float **EEta,**oldEEta;
{
 ...
 // fi2 = 2+v_trench*(3.14159/(11.1*180.0))*(200.0-age);
 ...
 *          element */
        ...
        for(jj = 1; jj <= vpts; jj++) {
            ...
            /* multiply the viscosity with this prefactor */
            //if(E->sx[m][3][E->ien[m][i].node[1]]>0.96) {
                EEta[m][ (i-1)*vpts + jj ] *= vmean;
                //fprintf(stderr,"decoupling_02 \n");
		/* Jiashun added */
	if ((fi-0.04) <= E->sx[m][2][E->ien[m][i].node[1]] && E->sx[m][2][E->ien[m][i].node[1]] <= (fi+0.04) && E->trace.ntracer_flavor[m][2][i]>0.0){
		EEta[m][(i-1)*vpts + jj] = 0.01;}

//        if ((fi2-0.04)<=E->sx[m][2][E->ien[m][i].node[1]] && E->sx[m][2][E->ien[m][i].node[1]]<=(fi2+0.04) && E->trace.ntracer_flavor[m][2][i]>0.0){
//		EEta[m][(i-1)*vpts + jj ] = 0.01;}
...
static void low_viscosity_channel_factor(struct All_variables *E, float *F)
{
    ...
    const int flavor = 1;
    double age, v_trench, rad_mean, rr, width=E->viscosity.lv_channel_thickness, flag;
    float phi_mean,fi;
    float find_age_in_MY();
    FILE *fp;
    ...
    //fprintf(stderr,"nz_min=%d, rad_min=%f\n",nz_min[m],E->sx[m][3][E->ien[m][nz_min[m]].node[1]]);
    //fprintf(stderr,"nz_max=%d, rad_max=%f\n",nz_max[m],E->sx[m][3][E->ien[m][nz_max[m]].node[1]]);
    ...
    for(m=1; m<=E->sphere.caps_per_proc; m++) 
		...
		if(flag == 2.0 || (phi_mean<fi+0.01 && phi_mean>fi-0.05 && flag==1.0)) {  
		  flag = 3.0;
		...
		if(flag == 3.0 && phi_mean>fi && phi_mean<fi+0.08) {
		  flag = 0.0;
		  /* loop over elements above e */
                  for(ii=i; ii<=E->lmesh.elz-1; ii++) {
		    ee = (k-1)*E->lmesh.elz + ii;
...
{
    int i, ii, k, m, e, ee, iii, jjj, node, nodeg;
    int nz_min[NCS], nz_max[NCS];
    const int flavor = 1;
    double rad_mean, rr, age;
        ...
        for(k=1; k<=E->lmesh.elx*E->lmesh.ely; k++) {
            for(i=nz_min[m]; i<=nz_max[m]; i++) {
                ...
                for(ii=i; ii>=nz_min[m]; ii--) {
                    ...
                    /* Jiashun add, to constrain the channel in subduction zone */
                    ...
                    if((E->trace.ntracer_flavor[m][flavor][ee] > 0) && (age == 0.0) &&
...
static void viscosity_filter(E,EEta)
    struct All_variables *E;
    float **EEta;
{
...
static void get_deformational_work(E,m,EEDOT)
struct All_variables *E;
float *EEDOT;
int m;
{
    ...
    for(e=1; e<=nel; e++) {
        ...
        for(j=1; j<=ppts; j++) {
            ...
            edot[2][2] += Vxyz[2][j];
            edot[3][3] += Vxyz[3][j];
            edot[1][2] += 0.5 * Vxyz[4][j];
            edot[1][3] += 0.5 * Vxyz[5][j];
            edot[2][3] += 0.5 * Vxyz[6][j];
        }
...
static void update_tracer_visc(E)
struct All_variables *E;
{
    void get_deformational_work();
    
    int m,e,kk,jj,ii;
    const int nel = E->lmesh.nel;
    const int ends = enodes[E->mesh.nsd];
    
    float theta,phi,rad,A,dA,A0,dt,D,Ev,Eh,T,Tsurf,H,M,P,temp;
    float *eedot,TT[9];
    double weight[9];
    ...
    if(E->viscosity.gdepv_visited) {
        for(m=1;m<=E->sphere.caps_per_proc;m++)  {
            get_deformational_work(E,m,eedot);
            for(kk=1;kk<=E->trace.ntracers[m];kk++) { /* loop through all tracers in serial */
                ...
                A = E->trace.extraq[m][1][kk];
                T = 0;
                dt = E->advection.fixed_timestep;
                ...
                if(e > 0) {
                    ...
                    for(jj=1;jj<=ends;jj++) {
                        TT[jj] = E->T[m][E->ien[m][e].node[jj]];
                    }
                    
                    for(jj=1;jj<=vpts;jj++) {
                        temp=0.0;
                        for(ii=1;ii<=ends;ii++)   {
                            temp += TT[ii] * E->N.vpt[GNVINDEX(ii,jj)];
                        }
                        T += temp*weight[jj];
                    }
                    
		    if(T<=0.5) { //&& E->trace.extraq[m][0][kk]==3) {
			dA = dt*(D*eedot[e]*exp(Ev/(T+Tsurf)-Ev/(1+Tsurf))*pow(A,-M)-H*exp(-Eh/(T+Tsurf)+Eh/(1+Tsurf))*pow(A,P));
			A = E->trace.extraq[m][2][kk]+dA;
		    }
		    else
			A = A0;
                    /* assign this element's viscosity as the 2nd extra property of tracer */
		    if(A<0.01) A=0.01;
                    E->trace.extraq[m][1][kk] = A;
                }/* end of if statement*/
            }/* end of kk loop */
        } /* end of m loop */
    }
        ...
        for(m=1;m<=E->sphere.caps_per_proc;m++)  {
            for(kk=1;kk<=E->trace.ntracers[m];kk++) {
                E->trace.extraq[m][1][kk] = A0;
                E->trace.extraq[m][2][kk] = A0;
                E->viscosity.gdepv_visited = 1;
	    }
	}
    }
...
static void visc_from_tracers(E,EEta)
struct All_variables *E;
float **EEta;
{
    int m,e,kk,jj;
    float r,phi,A_mean,A0=1,M=2;
    ...
    void update_tracer_visc();
    
    /* updat the grain size recorded in tracers */
    update_tracer_visc(E);
    ...
    for(m=1;m<=E->sphere.caps_per_proc;m++)
        for(e=1;e<=nel;e++)
            if(el_count[e]>0) {
                A_mean = A_count[e] / el_count[e];
		r= 0.5 * (E->sx[m][3][E->ien[m][e].node[1]] +
                                  E->sx[m][3][E->ien[m][e].node[8]]);
		phi= 0.5 * (E->sx[m][2][E->ien[m][e].node[1]] +
                                  E->sx[m][2][E->ien[m][e].node[8]]);
		if(r>=0.98)
                    for(jj=1;jj<=vpts;jj++) {
                        //if(oldEEta[m][(e-1)*vpts+jj] == 0.0)
                        if(A_mean<0.2) A_mean=0.2;
                        EEta[m][(e-1)*vpts+jj] *= pow(A_mean/A0,-M);
                    } /* end of jj loop */
            } /* end of if statement */
