...
void visc_from_T(E,EEta,propogate)
     ...
     int propogate;
{
    int m,i,j,k,l,z,jj,kk,imark,nz;
    float zero,e_6,one,eta0,Tave,depth,temp,tempa,temp1,TT[9],Tadi[129],newtemp[9];
    float zzz,zz[9],adiatemp;
    ...
    /* DJB, 05/02/08 */
    /* Viscosity structure 1 computed using temperature with adiabatic */
    /* gradient removed */

    case 7:			/* eta = N_0 exp( E * (T_0 - T))  */
        for(m=1;m<=E->sphere.caps_per_proc;m++)
            /* Loop over ELEMENTS */
            for(i=1;i<=nel;i++)   {
                l = E->mat[m][i];
                ...
                /* ien is node no. to element no.(s) mapping */
                for(kk=1;kk<=ends;kk++) {
                    TT[kk] = E->T[m][E->ien[m][i].node[kk]];
                    /* this is depth */
	            zz[kk] = (1.0 - E->sx[m][3][E->ien[m][i].node[kk]]);
                    ...
                    /* This is not the best way (should really use Tadi[i] */
                    /* formulation given in Material_properties.c */
                    adiatemp = (0.311488612+E->control.surface_temp) * exp(E->control.disptn_number * zz[kk]) - (0.311488612+E->control.surface_temp);

                    /* remove adiabatic gradient whilst still operating on nodes */
                    newtemp[kk] = TT[kk] - adiatemp;
                    newtemp[kk] *= 1.284156093; /* comp temp scale : incomp temp scale (also equivalent to comp temperature_cmb non-dim to super adia temp */

                    /* ensure that newtemp is not greater than one */
                    newtemp[kk] = min(newtemp[kk],one);

                    /* for debugging */
/*                    if (E->parallel.me==0) fprintf(stderr,"kk=%.1i, AbsT=%.3e, adiaT=%.3e, newT=%.3e, depth=%.3e \n",kk,TT[kk],adiatemp,newtemp[kk],zz[kk]); */

                }

                /* I think this part computes the element temperature */
                /* based on the shape function formulation */
                for(jj=1;jj<=vpts;jj++) {
                    temp=0.0;
                    for(kk=1;kk<=ends;kk++)   {
                        /* compute temp for rheol using newtemp */
                        temp += newtemp[kk] * E->N.vpt[GNVINDEX(kk,jj)];
                    }
    ...
    /* DJB, 08/11/08 */
    /* Viscosity structure 1 computed using refstate.Tadi gradient */
    /* gradient removed */

    case 8:			/* eta = N_0 exp( E * (T_0 - T))  */
        for(m=1;m<=E->sphere.caps_per_proc;m++)
            /* Loop over ELEMENTS */
            for(i=1;i<=nel;i++)   {
                l = E->mat[m][i];
                ...
                for(kk=1;kk<=ends;kk++) {
                    ...
                    /* get z node number */
                    nz = ((E->ien[m][i].node[kk]-1) % E->lmesh.noz)+1;

                    /* get temperature */
                    TT[kk] = E->T[m][E->ien[m][i].node[kk]];

                    /* remove adiabatic gradient whilst still operating on nodes */
                    newtemp[kk] = TT[kk] - E->refstate.Tadi[nz] + 0.377979236;
                    newtemp[kk] *= 1.322823985; /* comp temp scale : incomp temp scale (also equivalent to comp temperature_cmb non-dim to super adia temp */

                    /* ensure that newtemp is not greater than one */
                    /* filter to ensure energetic consistency? */
                    newtemp[kk] = min(newtemp[kk],one);
