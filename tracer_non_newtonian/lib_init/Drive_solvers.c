...
void general_stokes_solver(struct All_variables *E)
{
  ...
  void update_tracer_extraq();

  float vmag;

  double Udot_mag, dUdot_mag,omega[3];
  int m,count,i,j,k,kk;
  ...
  if (E->viscosity.SDEPV || E->viscosity.PDEPV || E->viscosity.GDEPV) {
	...
	/* update tracer extra quantities related to GDEPV */
	if (E->viscosity.GDEPV)
	  for(m=1;m<=E->sphere.caps_per_proc;m++)
	    for(kk=1;kk<=E->trace.ntracers[m];kk++)
	      E->trace.extraq[m][2][kk] = E->trace.extraq[m][1][kk];
	break;
      }
      
      get_system_viscosity(E,count,E->EVI[E->mesh.levmax],E->VI[E->mesh.levmax]);
      construct_stiffness_B_matrix(E);
      solve_constrained_flow_iterative(E);
