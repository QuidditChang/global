...
static void get_deformational_work(E,m,e,EEDOT,theta,phi,rad)
struct All_variables *E;
float EEDOT,theta,phi,rad;
int m,e;
{
    void get_global_shape_fn();
    ...
    /* edot is 2 * (the deviatoric strain rate tensor) */
    for(j=1; j<=ppts; j++) {
        edot[1][1] += Vxyz[1][j] * weight[j];
        edot[2][2] += Vxyz[2][j] * weight[j];
        edot[3][3] += Vxyz[3][j] * weight[j];
        edot[1][2] += 0.5 * Vxyz[4][j] * weight[j];
        edot[1][3] += 0.5 * Vxyz[5][j] * weight[j];
        edot[2][3] += 0.5 * Vxyz[6][j] * weight[j];
    }
    
    EEDOT = edot[1][1] * edot[1][1]
    + edot[1][2] * edot[1][2] * 2.0
    + edot[2][2] * edot[2][2]
    + edot[2][3] * edot[2][3] * 2.0
    + edot[3][3] * edot[3][3]
...
struct All_variables *E;
{
    ...
    if(E->viscosity.gdepv_visited) {
        for(m=1;m<=E->sphere.caps_per_proc;m++)  {
		    ...
		    get_deformational_work(E,m,e,eedot,theta,phi,rad);
		    ...
		    if(T<=0.9) {
			dA = dt*(D*eedot*exp(Ev/(T+Tsurf)-Ev/(1+Tsurf))*pow(A,-M)-H*exp(-Eh/(T+Tsurf)+Eh/(1+Tsurf))*pow(A,P));
...
float **EEta;
{
    ...
    fprintf(stderr,"visc_from_tracers\n");
