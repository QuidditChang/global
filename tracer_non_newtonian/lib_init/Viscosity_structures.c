 ...
 *          element */
		...
		/* Jiashun added */
	if((fi1-0.02)<=E->sx[m][2][E->ien[m][i].node[1]] && E->sx[m][2][E->ien[m][i].node[1]]<=(fi1+0.02) && E->trace.ntracer_flavor[m][2][i]>0.0) 
		EEta[m][ (i-1)*vpts + jj ] = 0.02;
                fprintf(stderr,"decoupling_03 \n");
