...
void visc_from_T(E,EEta,propogate)
     ...
     int propogate;
{
    int m,i,j,k,l,z,jj,kk,imark,iii,jjj,kkk,intage,node,nnn,ttt,count,bd_i,bd_j,bd_k;
		...
		flag2 = 0;
			...
			if(rheo_trick == 1) { //a strong slab in the upper mantle as a stress guide
						...
						if(rheo_trick == 9){
                                        ...
                                        if (E->sx[m][2][node]>fi-lon_dcr && E->sx[m][2][node]<fi+lon_inc && rheo_trick==8) {
                                            if(E->sx[m][2][node]<fi+0.1){
                                                if(E->sx[m][2][node]>fi && r>0.99){
                                                        EEta[m][ (i-1)*vpts + jj ] = temp2;
                                                }
                                                else if(r>0.975){
                                                        EEta[m][ (i-1)*vpts + jj ] = temp1;
                                            ...
                                            else if(E->sx[m][2][node]<fi+0.3){
                                                guide_dep = 0.15*(fi+0.1-E->sx[m][2][node])+0.99;
                                                if(r>=guide_dep && r<=guide_dep+0.03){
                                                        EEta[m][ (i-1)*vpts + jj ] = temp2;
                                                }
                                                else if(r>guide_dep-0.01 && r<guide_dep){
                                                        EEta[m][ (i-1)*vpts + jj ] = temp1;
                                                }
                                            }
                                            else if(E->sx[m][2][node]<fi+lon_inc){
                                                if(r>0.96 && r<0.98){
                                                        EEta[m][ (i-1)*vpts + jj ] = temp2;
                                                }
                                                else if(r>0.95 && r<0.96 && E->sx[m][2][node]<fi+lon_inc-0.5){
                                                        EEta[m][ (i-1)*vpts + jj ] = temp1;
                                                }
                                            }
                                        }

					if (E->sx[m][2][node]>fi-lon_dcr && E->sx[m][2][node]<fi+lon_dcr && r>0.985) {
                                            if(rheo_trick==7){
                                                EEta[m][ (i-1)*vpts + jj ] = temp1;
                                                //add in the internal velocity BC
                                                //E->sphere.cap[m].VB[2][node] = -10000; //E->sphere.cap[m].VB[2][node-iii+E->lmesh.noz];
                                                //E->sphere.cap[m].VB[1][node] = 5000; //E->sphere.cap[m].VB[1][node-iii+E->lmesh.noz];
                                            }
                                            if(rheo_trick==5 && r>1.0-0.025/lon_dcr*(E->sx[m][2][node]-(fi-lon_dcr)))
                                                EEta[m][ (i-1)*vpts + jj ] = temp1;
                                        }
                                        else if (E->sx[m][2][node]>fi && E->sx[m][2][node]<fi+lon_inc && r>0.975) {
                                          //  if(rheo_trick==7)
                                                //EEta[m][ (i-1)*vpts + jj ] = temp1*cos((E->sx[m][2][node]-fi)/lon_inc);
                                          //    EEta[m][ (i-1)*vpts + jj ] = temp1*((fi+lon_inc-E->sx[m][2][node])/lon_inc);
                                          //  if(rheo_trick==5)
                                                EEta[m][ (i-1)*vpts + jj ] = temp1;
                                        }
