...
static void pg_solver(struct All_variables *E,
                      ...
                      struct SOURCES Q0,
                      double diff, int bc, unsigned int **FLAGS, int step);
...
{
    int i,d,n,nel,el,node,m,a;

    float global_fmin();
    void velo_from_element();

    float adv_timestep,t1,t2;
	...
	/* smooth VV if too large */
        for(a=1;a<=ends;a++)   {
                t1 = sqrt(pow(VV[1][a],2)+pow(VV[2][a],2)+pow(VV[3][a],2));
                if(t1>10*E->data.scalev) {
                   t2 = (10*E->data.scalev)/t1;
                   VV[1][a] *= t2;
                   VV[2][a] *= t2;
                   VV[3][a] *= t2;
                }
...
void PG_timestep_solve_v303(struct All_variables *E)
{
  ...
  do {
	...
	pg_solver(E,E->T,E->Tdot,DTdot,E->convection.heat_sources,E->control.inputdiff,1,E->node,1);
  ...
  if(rheo_trick==-1) {
    for(m=1;m<=E->sphere.caps_per_proc;m++)
      for(i=1;i<=E->lmesh.nno;i++) 
          if(E->sphere.cap[m].V[3][i] > 500.0) 
              E->T[m][i] += (1.0-E->control.TBCbotval)*E->sphere.cap[m].V[3][i]/500000.0;
	  //else if(E->T[m][i] > E->control.TBCbotval)
	  //    E->T[m][i] -= (1.0-E->control.TBCbotval)*E->sphere.cap[m].V[3][i]/500000.0;
  }
...
void PG_timestep_solve(struct All_variables *E)
{
  ...
  void surface_z_bc();
  int i,m,psc_pass,iredo,t_iter,i_smth,flag,order;
  double time0,time1,T_interior1,tt1,tt2,ratio;
  double *DTdot[NCS], *T1[NCS], *Tdot1[NCS];
  FILE *fp,*fp1,*fp2;
  ...
  //T_interior1 = Tmaxd(E,E->T)-Tmind(E,E->T);
  //T_interior1 = E->control.lith_age_mantle_temp-Tmind(E,E->T);
  ...
  T_interior1 = T_max-T_min;

  //add another loop to make the actual time equal to fixed timestep
  timestep_elapsed = 0.0;
  t_iter = 0;
  total_time = 0.99*E->advection.fixed_timestep; 
  if(order) total_time = 9.9*E->advection.fixed_timestep;
  //if(order) total_time = 11.0*E->advection.fixed_timestep;
  
  do {
    E->advection.dt_reduced = 1.0;
    E->advection.last_sub_iterations = 0;
    E->advection.timestep = E->advection.fixed_timestep - timestep_elapsed;
    flag = 0;
    
    if(order && timestep_elapsed<0.9*total_time) 
        E->advection.timestep = E->advection.fixed_timestep;
    else if(order)
        E->advection.timestep = 10*E->advection.fixed_timestep - timestep_elapsed;
    ...
    do {
        ...
        if (E->advection.ADVECTION) {
            ...
            for(psc_pass=0;psc_pass<E->advection.temp_iterations;psc_pass++)   {
                pg_solver(E,E->T,E->Tdot,DTdot,E->convection.heat_sources,diff,1,E->node,1);
                corrector(E,E->T,E->Tdot,DTdot);
                temperatures_conform_bcs(E);
                //if(E->sphere.caps==1)  apply_smooth_sideTbc(E);
            }
        }                /* get the max temperature for new T */
        E->monitor.T_interior = Tmaxd(E,E->T)-Tmind(E,E->T);
        if(E->parallel.me==0){
            fprintf(E->fp,"E->monitor.T_interior=%f,T_interior1=%f,ratio=%f\n",E->monitor.T_interior,T_interior1,E->monitor.T_interior/T_interior1); 
            fflush(E->fp);
        }
        
        if (fabs(E->monitor.T_interior/T_interior1) > E->monitor.T_maxvaried && flag==0) {
              for(m=1;m<=E->sphere.caps_per_proc;m++)
                    for (i=1;i<=E->lmesh.nno;i++)   {
                        E->T[m][i] = T1[m][i];
                        E->Tdot[m][i] = Tdot1[m][i];
              ...
              //if( E->advection.timestep <= 0.025*E->advection.fixed_timestep || flag == 1) {
              if( E->advection.timestep <= 0.005*E->advection.fixed_timestep || flag == 1) {
                i_smth = 0;
                ...
                do {
                    ...
                    flag = 1;
                    if(i_smth==0) ratio = 10000;
                    else ratio = E->monitor.T_interior/T_interior1;
                    predictor(E,E->T,E->Tdot);
                    pg_solver(E,E->T,E->Tdot,DTdot,E->convection.heat_sources,diff,1,E->node,0);
                    corrector(E,E->T,E->Tdot,DTdot);
                    temperatures_conform_bcs(E);
                    //if(E->sphere.caps==1)  apply_smooth_sideTbc(E);
                    E->monitor.T_interior = Tmaxd(E,E->T)-Tmind(E,E->T);
                    if(E->parallel.me==0){
                        fprintf(E->fp,"Smooth_step %d: E->monitor.T_interior_new=%f,ratio=%f\n",i_smth,E->monitor.T_interior,E->monitor.T_interior/T_interior1);
                        fflush(E->fp);
                    }
                    i_smth++;
                } while (E->monitor.T_interior/T_interior1 > 1.01 && E->monitor.T_interior/T_interior1<ratio && i_smth < 10);
                E->advection.timestep = t_temp;
              }

        } //end of if (T_maxvaried)

    }  while ( iredo==1 && E->advection.last_sub_iterations <= 10);

    if (order == 0) filter_1(E,T_max,T_min);
    //if (order == 0) filter(E);

    timestep_elapsed += E->advection.timestep;
    t_iter ++;
    if(E->parallel.me==0){
        fprintf(E->fp,"E->advection.last_sub_iterations=%d, tsolver_iter=%d\n",E->advection.last_sub_iterations,t_iter);
        fprintf(E->fp,"E->advection.timestep=%e, Elapsed timestep=%e\n",E->advection.timestep,timestep_elapsed);
        fflush(E->fp);
    }
  //} while (timestep_elapsed < 0.99*E->advection.fixed_timestep && t_iter < 100);
  } while (timestep_elapsed < total_time && t_iter < 100);
  ...
  if (E->advection.last_sub_iterations==10)
    E->control.keep_going = 0;
...
void filter_1(struct All_variables *E, double tmx, double tmn)
{
        double Tsum0,Tmin,Tmax,Tsum1,TDIST,TDIST1;
        int m,i,TNUM,TNUM1;
        double Tmax1,Tmin1;
        ...
        for(m=1;m<=E->sphere.caps_per_proc;m++)
                for(i=1;i<=E->lmesh.nno;i++)  {
                    if(E->sx[m][3][i] < 1.1){
                        ...
                        if(E->T[m][i]<Tmin)  Tmin=E->T[m][i];
                        if(E->T[m][i]<tmn) E->T[m][i]=tmn;
                        if(E->T[m][i]>Tmax) Tmax=E->T[m][i];
                        if(E->T[m][i]>tmx) E->T[m][i]=tmx;
                    }

                }

        if(Tmin==0.0) Tmin=tmn;
        if(Tmax==0.0) Tmax=tmx;

        MPI_Allreduce(&Tmin,&Tmin1,1,MPI_DOUBLE,MPI_MIN,E->parallel.world);
        MPI_Allreduce(&Tmax,&Tmax1,1,MPI_DOUBLE,MPI_MAX,E->parallel.world);

        //fprintf(stderr,"Tmax= %f, Tmin= %f, Tmax1= %f, Tmin1= %f\n",Tmax,Tmin,Tmax1,Tmin1);

        for(m=1;m<=E->sphere.caps_per_proc;m++)
                for(i=1;i<=E->lmesh.nno;i++)  {
                    if(E->sx[m][3][i] < 1.1){
                        //if(E->T[m][i]<(Tmin1))   E->T[m][i]=Tmin1;
                        if(E->T[m][i]<(2*tmn-Tmin1))   E->T[m][i]=tmn;
                        if(E->T[m][i]>(2*tmx-Tmax1))   E->T[m][i]=tmx;
                        if (!(E->NODE[lev][m][i] & SKIP))  {
                                Tsum1+=E->T[m][i];
                                if(E->T[m][i]!=tmn && E->T[m][i]!=tmx && fabs(E->T[m][i]-E->control.lith_age_mantle_temp)>=0.0001) TNUM++; //&& fabs(E->T[m][i]-E->control.lith_age_mantle_temp)>=0.0001)
                        }
                    }
                }
        ...
        //fprintf(stderr,"TDIST= %f, TNUM= %f\n",TDIST,TNUM);

        for(m=1;m<=E->sphere.caps_per_proc;m++)
                for(i=1;i<=E->lmesh.nno;i++)   {
                        //if(E->T[m][i]!=tmn && E->T[m][i]!=tmx)
                        if(E->T[m][i]!=tmn && E->T[m][i]!=tmx && fabs(E->T[m][i]-E->control.lith_age_mantle_temp)>=0.0001)
                                E->T[m][i] +=TDIST; // && fabs(E->T[m][i]-E->control.lith_age_mantle_temp)>=0.0001
                }
...
static void apply_smooth_sideTbc(E)
    struct All_variables *E;

{
  int i,m;
  float theta,phi,r,g,gap,gap1,ramp;
  unsigned int type;
  ...
  for(m=1; m<=E->sphere.caps_per_proc;m++)
     for(i=1;i<=E->lmesh.nno;i++) {
        ...
        phi = E->sx[m][2][i];
        r = E->sx[m][3][i];
        g = E->T[m][i] - E->control.TBCbotval;
	gap = 0.2;
        ramp = 0.17;
        //gap = 0.2;
        //ramp = 0.1;
        //if(E->parallel.me_loc[3]<E->parallel.nprocz-1 && r>0.998) { //this is the sub-surface layer
        //if(r>0.84) {
        if(theta > E->control.theta_max-gap)
             if(theta > E->control.theta_max-ramp)
               g = 0.0;
             else {
               g = g+(0.0-g)*(theta-(E->control.theta_max-gap))/(gap-ramp);
             }
        if(theta < E->control.theta_min+gap)
             if(theta < E->control.theta_min+ramp)
               g = 0.0;
             else {
               g = g+(0.0-g)*((E->control.theta_min+gap)-theta)/(gap-ramp);
             }
        if(phi > E->control.fi_max-gap)
             if(phi > E->control.fi_max-ramp)
               g = 0.0;
             else {
               g = g+(0.0-g)*(phi-(E->control.fi_max-gap))/(gap-ramp);
             }
        if(phi < E->control.fi_min+gap)
             if(phi < E->control.fi_min+ramp)
               g = 0.0;
             else {
               g = g+(0.0-g)*((E->control.fi_min+gap)-phi)/(gap-ramp);
             }

        E->T[m][i] = g + E->control.TBCbotval;
        ...
        switch (type) {
            case 0:  // no match, next node 
                break;
            case TBX:
                E->sphere.cap[m].TB[1][i] = E->T[m][i];
                break;
            case TBZ:
                E->sphere.cap[m].TB[3][i] = E->T[m][i];
                break;
            case TBY:
                E->sphere.cap[m].TB[2][i] = E->T[m][i];
                break;
        }*/
